define(['angular'], function () {
    ...
    app.factory('configService', function () {
        ...
        //var rootUrlApi = 'http://' + hostname + ':2912';
        if (!port) {
        ...
        result.buildLayouts = function (folder) {
            return this.rootUrlWeb + "/_layouts/15/BTS.SP.STC.PHF/views/" + folder + "/";
        };

        result.buildUrl = function (folder, file) {
            var d = new Date();
            return this.rootUrlWeb + "/_layouts/15/BTS.SP.STC.PHF/views/" + folder + "/" + file + ".html?time=" + d.getTime();
        };
        result.pageDefault = {
            totalItems: 0,
            itemsPerPage: 20,
