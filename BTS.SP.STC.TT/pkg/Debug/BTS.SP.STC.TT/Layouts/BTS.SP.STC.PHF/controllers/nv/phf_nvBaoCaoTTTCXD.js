define(['controllers/auth/AuController', 'controllers/dm/phf_dmDoiTuong', 'controllers/dm/phf_dmDotThanhTra'], function () {
    'use strict';
    var app = angular.module('phf_nvBaoCaoTTTCXD', ['authModule', 'phf_dmDoiTuong', 'phf_dmDotThanhTra']);
    var list = [];
    app.factory('BaoCaoTTTCXDService', ['$http', 'configService', 'Upload', function ($http, configService, Upload) {
        var serviceUrl = configService.rootUrlWebApi + '/nv/phf_nvBaoCaoTTTCXD';
        var result = {
    ...
    app.controller('BaoCaoTTTCXDViewCtrl', ['$scope', '$location', 'BaoCaoTTTCXDService', 'configService', '$state', 'tempDataService', '$filter', '$uibModal', '$log', 'securityService', 'toaster', 'ngNotify', 'blockModalService', 'CacheFactory', 'DmDoiTuongService', 'DmDotThanhTraService',
    function ($scope, $location, service, configService, $state, tempDataService, $filter, $uibModal, $log, securityService, toaster, ngNotify, blockModalService, CacheFactory, dmDoiTuongService, dmDotThanhTraService) {
        $scope.title = function () {
            return 'Báo cáo thanh tra tài chính - xây dựng';
        };
        $scope.config = {
        ...
        $scope.listReport = [
        {
            id: 1,
            value: 'rp_TTTC_XD',
            text: 'Báo cáo thanh tra tài chính - xây dựng',
            description: 'Báo cáo thanh tra tài chính - xây dựng',
            parent: null,
            expanded: true,
            ...
            items: [
                {
                    id: 2,
                    value: 'rp01_TTTC_XD',
                    text: 'Biểu số 01/TTTC-XD',
                    description: 'THỦ TỤC, THẨM QUYỀN PHÊ DUYỆT TỔNG MỨC ĐẦU TƯ',
                    template: 'TEMPLATE_BM_01TT_TCXD',
                    parent: 1,
                ...
                },
                {
                    id: 3,
                    value: 'rp02_TTTC_XD',
                    text: 'Biểu số 02/TTTC-XD ',
                    description: 'TỔNG HỢP SAI PHẠM TRONG XÂY DỰNG, PHÊ DUYỆT TỔNG MỨC ĐẦU TƯ',
                    parent: 1,
                    template: 'TEMPLATE_BM_02TT_TCXD',
                    spriteCssClass: "folder"
                },
                {
                    id: 4,
                    value: 'rp03_TTTC_XD',
                    text: 'Biểu số 03/TTTC-XD',
                    description: 'TỔNG HỢP TÌNH HÌNH HUY ĐỘNG, BỐ TRÍ, GIẢI NGÂN VỐN ĐẦU TƯ',
                    parent: 1,
                    template: 'TEMPLATE_BM_03TT_TCXD',
                    spriteCssClass: "folder"
                },
                {
                    id: 5,
                    value: 'rp04_TTTC_XD',
                    text: 'Biểu số 04/TTTC-XD',
                    description: 'TỔNG HỢP CÁC SAI PHẠM HUY ĐỘNG, BỐ TRÍ, GIẢI NGÂN VỐN',
                    template: 'TEMPLATE_BM_04TT_TCXD',
                    parent: 1,
                    spriteCssClass: "folder"
                },
                {
                    id: 6,
                    value: 'rp05_TTTC_XD',
                    text: 'Biểu số 05/TTTC-XD',
                    description: 'THỦ TỤC, THẨM QUYỀN PHÊ DUYỆT DỰ TOÁN',
                    template: 'TEMPLATE_BM_05TT_TCXD',
                    parent: 1,
                    spriteCssClass: "folder"
                },
                {
                    ...
                    description: 'TỔNG HỢP CÁC SAI PHẠM KHỐI LƯỢNG, ĐƠN GIÁ, ĐỊNH MỨC DỰ TOÁN',
                    parent: 1,
                    template: 'TEMPLATE_BM_06TT_TCXD',
                    spriteCssClass: "folder"
                },
                {
                    id: 8,
                    value: 'rp07_TTTC_XD',
                    text: 'Biểu số 07/TTTC-XD',
                    description: 'TỔNG HỢP TÌNH HÌNH THỰC HIỆN THEO HỢP ĐỒNG CÁC GÓI THẦU, HẠNG MỤC CÔNG TRÌNH',
                    template: 'TEMPLATE_BM_07TT_TCXD',
                    parent: 1,
                    spriteCssClass: "folder"
                },
                {
                    id: 9,
                    value: 'rp08_TTTC_XD',
                    text: 'Biểu số 08/TTTC-XD',
                    description: 'THỐNG KÊ TÌNH HÌNH NGHIỆM THU, THANH TOÁN GIÁ TRỊ KHỐI LƯỢNG HOÀN THÀNH',
                    parent: 1,
                    template: 'TEMPLATE_BM_08TT_TCXD',
                    spriteCssClass: "folder"
                },
                {
                    id: 10,
                    value: 'rp10_TTTC_XD',
                    text: 'Biểu số 10/TTTC-XD',
                    description: 'TỔNG HỢP TÌNH HÌNH QUYẾT TOÁN DỰ ÁN',
                    parent: 1,
                    template: 'TEMPLATE_BM_10TT_TCXD',
                    spriteCssClass: "folder"
                }
            ]
        }
        ...
        $("#dropdowntree").kendoDropDownTree({
            ...
            change: function (e) {
                ...
                console.log('$scope.reportName', $scope.reportName);
        ...
        var tree = $("#dropdowntreeDonVi").kendoDropDownTree({
            ...
            change: function (e) {
                var choiceValue = this.value();
                $scope.donVi = choiceValue;
                if ($scope.listDonVi && $scope.listDonVi.length > 0) {
                    var unitCode = $filter('filter')($scope.listDonVi, { Value: choiceValue }, true);
                    console.log(unitCode);
                    if (unitCode && unitCode.length === 1) {
                        $scope.filtered.AdvanceData.UnitCode = unitCode[0].Value;
                        scope.filtered.IsAdvance = true;
                    }
                    // else {
                    // angular.forEach($scope.listDonVi, function (v, k) {
                    // if (v && v.spriteCssClass == 'rootfolder' && v.items && v.items.length > 0) {
                    // var unitCode = $filter('filter')(v.items, { value: choiceValue }, true);
                    // if (sub && sub.length == 1) {
                    // $scope.filtered.AdvanceData.UnitCode = sub[0].value;
                    // $scope.filtered.IsAdvance = true;
                    // }
                    // }
                    // });
        ...
        var tree = $("#dropdowntreeKyBaoCao").kendoDropDownTree({
            ...
            change: function (e) {
                ...
                console.log(' $scope.kyThanhTra', $scope.kyThanhTra);
        ...
        function loadAccessList() {
            securityService.getAccessList('phf_BaoCaoTTTCXD').then(function (successRes) {
        ...
        $scope.importExcel = function () {
            if (!$scope.kyThanhTra) {
                ...
                var modalInstanceImport = $uibModal.open({
                    backdrop: 'static',
                    size: 'sm',
                    templateUrl: configService.buildUrl('nv/phf_nvBaoCaoTTTCXD', 'importFile'),
                    controller: 'BaoCaoTTTCXDImportFileCtrl',
                    resolve: {
                        kyThanhTra: function () {
        ...
        $scope.create = function () {
            var modalInstance = $uibModal.open({
                backdrop: 'static',
                size: 'md',
                templateUrl: configService.buildUrl('nv/phf_nvBaoCaoTTTCXD', 'add'),
                controller: 'BaoCaoTTTCXDCreateCtrl',
                resolve: {}
            });
        ...
        $scope.detail = function (target) {
            var modalInstance = $uibModal.open({
                backdrop: 'static',
                templateUrl: configService.buildUrl('nv/phf_nvBaoCaoTTTCXD', 'detail'),
                controller: 'BaoCaoTTTCXDDetailCtrl',
                resolve: {
                    targetData: function () {
                        return target;
                    }
        ...
        $scope.delete = function (target) {
            var modalInstance = $uibModal.open({
                backdrop: 'static',
                templateUrl: configService.buildUrl('nv/phf_nvBaoCaoTTTCXD', 'delete'),
                controller: 'BaoCaoTTTCXDDeleteCtrl',
                resolve: {
                    targetData: function () {
                        return target;
                    }
        ...
        $scope.edit = function (target) {
            var modalInstance = $uibModal.open({
                backdrop: 'static',
                templateUrl: configService.buildUrl('nv/phf_nvBaoCaoTTTCXD', 'edit_' + target.MA_FILE),
                controller: 'BaoCaoTTTCXDEditCtrl',
    ...
    app.controller('BaoCaoTTTCXDImportFileCtrl', ['$scope', '$uibModalInstance', '$location', 'BaoCaoTTTCXDService', 'configService', '$state', 'tempDataService', '$filter', '$uibModal', '$log', 'ngNotify', 'kyThanhTra', 'reportName', 'donVi', 'doiTuong', 'blockModalService', 'userService',
    ...
    app.controller('BaoCaoTTTCXDDetailCtrl', ['$scope', '$uibModalInstance', 'BaoCaoTTTCXDService', 'configService', '$state', 'tempDataService', '$filter', '$uibModal', '$log', 'ngNotify', 'targetData', 'userService',
    ...
    app.controller('BaoCaoTTTCXDDeleteCtrl', ['$scope', '$uibModalInstance', 'BaoCaoTTTCXDService', 'configService', '$state', 'tempDataService', '$filter', '$uibModal', '$log', 'ngNotify', 'targetData', 'userService',
     function ($scope, $uibModalInstance, service, configService, $state, tempDataService, $filter, $uibModal, $log, ngNotify, targetData, userService) {
         ...
         $scope.title = function () { return 'Xóa báo cáo thanh tra tài chính - xây dựng'; };
    ...
    app.controller('BaoCaoTTTCXDEditCtrl', ['$scope', '$uibModalInstance', 'BaoCaoTTTCXDService', 'configService', '$state', 'tempDataService', '$filter', '$uibModal', '$log', 'ngNotify', 'targetData', 'userService', 'DmDoiTuongService',
       function ($scope, $uibModalInstance, service, configService, $state, tempDataService, $filter, $uibModal, $log, ngNotify, targetData, userService, dmDoiTuongService) {
           $scope.config = angular.copy(configService);
           $scope.tempData = tempDataService.tempData;
           $scope.target = angular.copy(targetData);
           $scope.isLoading = false;
           ...
           $scope.save = function () {
               if ($scope.target.MA_FILE == 'TEMPLATE_BM_01TT_TCXD') {
                   $scope.target.DETAIL = $scope.data.PHF_BM_01TT_TCXD_DTO;
               }
               else if ($scope.target.MA_FILE == 'TEMPLATE_BM_02TT_TCXD') {
                   $scope.target.DETAIL = $scope.data.PHF_BM_02TT_TCXD_DTO;
               }
               else if ($scope.target.MA_FILE == 'TEMPLATE_BM_03TT_TCXD') {
                   $scope.target.DETAIL = $scope.data.PHF_BM_03TT_TCXD_DTO;
               }
               else if ($scope.target.MA_FILE == 'TEMPLATE_BM_04TT_TCXD') {
                   $scope.target.DETAIL = $scope.data.PHF_BM_04TT_TCXD_DTO;
               }
               else if ($scope.target.MA_FILE == 'TEMPLATE_BM_05TT_TCXD') {
                   $scope.target.DETAIL = $scope.data.PHF_BM_05TT_TCXD_DTO;
               }
               else if ($scope.target.MA_FILE == 'TEMPLATE_BM_06TT_TCXD') {
                   $scope.target.DETAIL = $scope.data.PHF_BM_06TT_TCXD_DTO;
               }
               else if ($scope.target.MA_FILE == 'TEMPLATE_BM_07TT_TCXD') {
                   $scope.target.DETAIL = $scope.data.PHF_BM_07TT_TCXD_DTO;
               }
               else if ($scope.target.MA_FILE == 'TEMPLATE_BM_08TT_TCXD') {
                   $scope.target.DETAIL = $scope.data.PHF_BM_08TT_TCXD_DTO;
               }
               else if ($scope.target.MA_FILE == 'TEMPLATE_BM_10TT_TCXD') {
                   $scope.target.DETAIL = $scope.data.PHF_BM_10TT_TCXD_DTO;
               }
           ...
           function loadDoiTuong() {
               dmDoiTuongService.getSelectData().then(function (response) {
                   if (response && response.status === 200) {
                       $scope.listDoiTuong = response.data;
                       console.log('$scope.listDoiTuong', $scope.listDoiTuong);
                       tempDataService.putTempData('listDoiTuong', $scope.listDoiTuong);
                       if ($scope.listDoiTuong != null) {
                           $scope.target.listDoiTuong = $scope.listDoiTuong[0].value
                       }
