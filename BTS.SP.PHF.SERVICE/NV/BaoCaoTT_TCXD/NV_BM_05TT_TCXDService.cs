...
{
    public interface INV_BM_05TT_TCXDService : IBaseService<PHF_BM_05TT_TCXD>
    {
    ...
    public class NV_BM_05TT_TCXDService : BaseService<PHF_BM_05TT_TCXD>, INV_BM_05TT_TCXDService
    {
        private readonly IRepositoryAsync<PHF_BM_05TT_TCXD> _repository;
        public NV_BM_05TT_TCXDService(IRepositoryAsync<PHF_BM_05TT_TCXD> repository) : base(repository)
        {
        ...
        public string UploadFileReport(HttpRequest request, string currentUser, string unitCode, string report, string period, string doiTuong)
        {
            try
            {
                List<PHF_BM_05TT_TCXD> listInsert = new List<PHF_BM_05TT_TCXD>();
                ...
                using (var excelPackage = new ExcelPackage(HttpContext.Current.Request.Files[0].InputStream))
                {
                    ...
                    if (workSheet != null)
                    {
                        ...
                        if (workSheet.Cells[rowHeader, 1] != null)
                        {
                            ...
                            {
                                bmfile_ncdp.TEN_BIEUMAU = "Biểu mẫu số: 05/TTTC-XD";
                            }
                        }
                        ...
                        if (workSheet.Cells[rowTitle, 1] != null)
                        {
                            ...
                            {
                                bmfile_ncdp.TIEUDE_BIEUMAU = "THỦ TỤC, THẨM QUYỀN PHÊ DUYỆT DỰ TOÁN";
                            }
                        }
                        ...
                        while (workSheet.Cells[startRow, 1].Value != null && !string.IsNullOrEmpty(workSheet.Cells[startRow, 1].Value.ToString()))
                        {
                            ...
                            {
                                PHF_BM_05TT_TCXD detail = new PHF_BM_05TT_TCXD()
                                {
                                    ObjectState = ObjectState.Added,
                                ...
                                detail.TEN_DUTOAN = workSheet.Cells[startRow, 2] != null ? workSheet.Cells[startRow, 2].Value?.ToString() : null;
                                detail.GIATRI = workSheet.Cells[startRow, 3] != null ? workSheet.Cells[startRow, 3].Value?.ToString() : null;
                                detail.SO = workSheet.Cells[startRow, 4] != null ? workSheet.Cells[startRow, 4].Value?.ToString() : null;
                                detail.NGAY = workSheet.Cells[startRow, 5] != null ? workSheet.Cells[startRow, 5].Value?.ToString() : null;
                                detail.THAMQUYEN_DUYET = workSheet.Cells[startRow, 6] != null ? workSheet.Cells[startRow, 6].Value?.ToString() : null;
                                detail.THUTUC_DACO = workSheet.Cells[startRow, 7] != null ? workSheet.Cells[startRow, 7].Value?.ToString() : null;
                                detail.NGUYENNHAN_THIEU = workSheet.Cells[startRow, 8] != null ? workSheet.Cells[startRow, 8].Value?.ToString() : null;
