...
{
    public interface INV_BM_02TT_TCXDService : IBaseService<PHF_BM_02TT_TCXD>
    {
    ...
    public class NV_BM_02TT_TCXDService : BaseService<PHF_BM_02TT_TCXD>, INV_BM_02TT_TCXDService
    {
        private readonly IRepositoryAsync<PHF_BM_02TT_TCXD> _repository;
        public NV_BM_02TT_TCXDService(IRepositoryAsync<PHF_BM_02TT_TCXD> repository) : base(repository)
        {
        ...
        public string UploadFileReport(HttpRequest request, string currentUser, string unitCode, string report, string period, string doiTuong)
        {
            try
            {
                List<PHF_BM_02TT_TCXD> listInsert = new List<PHF_BM_02TT_TCXD>();
                ...
                using (var excelPackage = new ExcelPackage(HttpContext.Current.Request.Files[0].InputStream))
                {
                    ...
                    if (workSheet != null)
                    {
                        ...
                        if (workSheet.Cells[rowHeader, 1] != null)
                        {
                            ...
                            {
                                bmfile_ncdp.TEN_BIEUMAU = "Biểu số 02/TTTC-XD";
                            }
                        }
                        ...
                        if (workSheet.Cells[rowTitle, 1] != null)
                        {
                            ...
                            {
                                bmfile_ncdp.TIEUDE_BIEUMAU = "TỔNG HỢP SAI PHẠM TRONG XÂY DỰNG, PHÊ DUYỆT TỔNG MỨC ĐẦU TƯ";
                            }
                        }
                        ...
                        while (workSheet.Cells[startRow, 1].Value != null && !string.IsNullOrEmpty(workSheet.Cells[startRow, 1].Value.ToString()))
                        {
                            ...
                            {
                                PHF_BM_02TT_TCXD detail = new PHF_BM_02TT_TCXD()
                                {
                                    ObjectState = ObjectState.Added,
                                ...
                                detail.TEN_SAIPHAM = workSheet.Cells[startRow, 2] != null ? workSheet.Cells[startRow, 2].Value?.ToString() : null;
                                detail.GIATRI = workSheet.Cells[startRow, 3] != null ? workSheet.Cells[startRow, 3].Value?.ToString() : null;
                                detail.NGUYENNHAN = workSheet.Cells[startRow, 4] != null ? workSheet.Cells[startRow, 4].Value?.ToString() : null;
                                detail.TRACHNHIEM = workSheet.Cells[startRow, 5] != null ? workSheet.Cells[startRow, 5].Value?.ToString() : null;
