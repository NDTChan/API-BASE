...
{
    public interface INV_BM_04TT_TCXDService : IBaseService<PHF_BM_04TT_TCXD>
    {
    ...
    public class NV_BM_04TT_TCXDService : BaseService<PHF_BM_04TT_TCXD>, INV_BM_04TT_TCXDService
    {
        private readonly IRepositoryAsync<PHF_BM_04TT_TCXD> _repository;
        public NV_BM_04TT_TCXDService(IRepositoryAsync<PHF_BM_04TT_TCXD> repository) : base(repository)
        {
        ...
        public string UploadFileReport(HttpRequest request, string currentUser, string unitCode, string report, string period, string doiTuong)
        {
            try
            {
                List<PHF_BM_04TT_TCXD> listInsert = new List<PHF_BM_04TT_TCXD>();
                ...
                using (var excelPackage = new ExcelPackage(HttpContext.Current.Request.Files[0].InputStream))
                {
                    ...
                    if (workSheet != null)
                    {
                        ...
                        if (workSheet.Cells[rowHeader, 1] != null)
                        {
                            ...
                            {
                                bmfile_ncdp.TEN_BIEUMAU = "Biểu mẫu số: 04/TTTC-XD";
                            }
                        }
                        ...
                        if (workSheet.Cells[rowTitle, 1] != null)
                        {
                            ...
                            {
                                bmfile_ncdp.TIEUDE_BIEUMAU = "TỔNG HỢP CÁC SAI PHẠM HUY ĐỘNG, BỐ TRÍ, GIẢI NGÂN VỐN";
                            }
                        }
                        ...
                        while (workSheet.Cells[startRow, 1].Value != null && !string.IsNullOrEmpty(workSheet.Cells[startRow, 1].Value.ToString()))
                        {
                            ...
                            {
                                PHF_BM_04TT_TCXD detail = new PHF_BM_04TT_TCXD()
                                {
                                    ObjectState = ObjectState.Added,
                                ...
                                detail.KETQUA = workSheet.Cells[startRow, 5] != null ? workSheet.Cells[startRow, 5].Value?.ToString() : null;
