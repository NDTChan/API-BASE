...
{
    public interface INV_BM_10TT_TCXDService : IBaseService<PHF_BM_10TT_TCXD>
    {
    ...
    public class NV_BM_10TT_TCXDService : BaseService<PHF_BM_10TT_TCXD>, INV_BM_10TT_TCXDService
    {
        private readonly IRepositoryAsync<PHF_BM_10TT_TCXD> _repository;
        public NV_BM_10TT_TCXDService(IRepositoryAsync<PHF_BM_10TT_TCXD> repository) : base(repository)
        {
        ...
        public string UploadFileReport(HttpRequest request, string currentUser, string unitCode, string report, string period, string doiTuong)
        {
            try
            {
                List<PHF_BM_10TT_TCXD> listInsert = new List<PHF_BM_10TT_TCXD>();
                ...
                using (var excelPackage = new ExcelPackage(HttpContext.Current.Request.Files[0].InputStream))
                {
                    ...
                    if (workSheet != null)
                    {
                        ...
                        if (workSheet.Cells[rowHeader, 1] != null)
                        {
                            ...
                            {
                                bmfile_ncdp.TEN_BIEUMAU = "Biểu mẫu số: 10/TTTC-XD";
                            }
                        }
                        ...
                        if (workSheet.Cells[rowTitle, 1] != null)
                        {
                            ...
                            {
                                bmfile_ncdp.TIEUDE_BIEUMAU = "TỔNG HỢP TÌNH HÌNH QUYẾT TOÁN DỰ ÁN";
                            }
                        }
                        ...
                        while (workSheet.Cells[startRow, 1].Value != null && !string.IsNullOrEmpty(workSheet.Cells[startRow, 1].Value.ToString()))
                        {
                            ...
                            {
                                PHF_BM_10TT_TCXD detail = new PHF_BM_10TT_TCXD()
                                {
                                    ObjectState = ObjectState.Added,
                                ...
                                detail.THOIGIAN_QUYETTOAN = workSheet.Cells[startRow, 3] != null ? workSheet.Cells[startRow, 3].Value?.ToString() : null;
                                detail.GIATRI_DENGHI = workSheet.Cells[startRow, 4] != null ? workSheet.Cells[startRow, 4].Value?.ToString() : null;
                                detail.GIATRI_XACDINH = workSheet.Cells[startRow, 5] != null ? workSheet.Cells[startRow, 5].Value?.ToString() : null;
                                detail.CHENHLECH = workSheet.Cells[startRow, 6] != null ? workSheet.Cells[startRow, 6].Value?.ToString() : null;
                                detail.THOIGIAN_QUYETTOAN_CHAM = workSheet.Cells[startRow, 7] != null ? workSheet.Cells[startRow, 7].Value?.ToString() : null;
                                detail.NGUYENNHAN = workSheet.Cells[startRow, 8] != null ? workSheet.Cells[startRow, 8].Value?.ToString() : null;
                                detail.GHICHU = workSheet.Cells[startRow, 9] != null ? workSheet.Cells[startRow, 9].Value?.ToString() : null;
