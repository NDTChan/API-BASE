...
{
    public interface INV_BM_16TT_NSDPService : IBaseService<PHF_BM_16TT_NSDP>
    {
        string UploadFileReport(HttpRequest request, string currentUser, string unitCode, string report, string period);
    }
    public class NV_BM_16TT_NSDPService : BaseService<PHF_BM_16TT_NSDP>, INV_BM_16TT_NSDPService
    {
        private readonly IRepositoryAsync<PHF_BM_16TT_NSDP> _repository;
        public NV_BM_16TT_NSDPService(IRepositoryAsync<PHF_BM_16TT_NSDP> repository) : base(repository)
        {
        ...
        public string UploadFileReport(HttpRequest request, string currentUser, string unitCode, string report, string period)
        {
            try
            {
                List<PHF_BM_16TT_NSDP> listInsert = new List<PHF_BM_16TT_NSDP>();
                ...
                using (var excelPackage = new ExcelPackage(HttpContext.Current.Request.Files[0].InputStream))
                {
                    ...
                    if (workSheet != null)
                    {
                        ...
                        {
                            if (!string.IsNullOrEmpty(workSheet.Cells[rowHeader, 5].Value?.ToString()))
                            {
                                bmfile_ncdp.TEN_BIEUMAU = workSheet.Cells[rowHeader, 5].Value?.ToString();
                            }
                            else
                            {
                                bmfile_ncdp.TEN_BIEUMAU = "Biểu số 16/TTTC-NSĐP";
                            }
                        }
                        int rowTitle = 2;
                        if (workSheet.Cells[rowTitle, 2] != null)
                        {
                            ...
                            {
                                bmfile_ncdp.TIEUDE_BIEUMAU = "BẢNG TỔNG HỢP SAI PHẠM, KHUYẾT ĐIỂM QUẢN LÝ CHI THƯỜNG XUYÊN";
                            }
                        }
                        int startRow = 6;
                        int stt = 1;
                        ...
                        while (workSheet.Cells[startRow, 1].Value != null && !string.IsNullOrEmpty(workSheet.Cells[startRow, 1].Value.ToString()))
                        {
                            ...
                            {
                                PHF_BM_16TT_NSDP detail = new PHF_BM_16TT_NSDP()
                                {
                                    ObjectState = ObjectState.Added,
                                ...
                                detail.DOITUONG = workSheet.Cells[startRow, 2] != null ? workSheet.Cells[startRow, 2].Value?.ToString() : null;
                                ...
                                detail.NOIDUNG = workSheet.Cells[startRow, 3] != null ? workSheet.Cells[startRow, 3].Value?.ToString() : null;
                                detail.NOIDUNG_SOTIEN = workSheet.Cells[startRow, 4] != null ? workSheet.Cells[startRow, 4].Value?.ToString() : null;
                                detail.NGUYENNHAN = workSheet.Cells[startRow, 5] != null ? workSheet.Cells[startRow, 5].Value?.ToString() : null;
