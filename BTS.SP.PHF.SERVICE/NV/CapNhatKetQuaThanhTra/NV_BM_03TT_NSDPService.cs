...
{
    public interface INV_BM_03TT_NSDPService : IBaseService<PHF_BM_03TT_NSDP>
    {
        string UploadFileReport(HttpRequest request, string currentUser, string unitCode, string report, string period);
    }
    public class NV_BM_03TT_NSDPService : BaseService<PHF_BM_03TT_NSDP>, INV_BM_03TT_NSDPService
    {
        private readonly IRepositoryAsync<PHF_BM_03TT_NSDP> _repository;
        public NV_BM_03TT_NSDPService(IRepositoryAsync<PHF_BM_03TT_NSDP> repository) : base(repository)
        {
        ...
        public string UploadFileReport(HttpRequest request, string currentUser, string unitCode, string report, string period)
        {
            try
            {
                List<PHF_BM_03TT_NSDP> listInsert = new List<PHF_BM_03TT_NSDP>();
                ...
                using (var excelPackage = new ExcelPackage(HttpContext.Current.Request.Files[0].InputStream))
                {
                    ...
                    if (workSheet != null)
                    {
                        ...
                        if (workSheet.Cells[rowHeader, 6] != null)
                        {
                            ...
                            {
                                bmfile_ncdp.TEN_BIEUMAU = "Biểu số 01/TTTC-NSĐP";
                            }
                        }
                        ...
                        if (workSheet.Cells[rowTitle, 1] != null)
                        {
                            ...
                            {
                                bmfile_ncdp.TIEUDE_BIEUMAU = "TỔNG HỢP SAI PHẠM, KHUYẾT ĐIỂM TRONG TRIỂN KHAI BAN HÀNH CHẾ ĐỘ CHÍNH SÁCH CỦA ĐỊA PHƯƠNG";
                            }
                        }
                        int startRow = 7;
                        int stt = 1;
                        ...
                        while (workSheet.Cells[startRow, 1].Value != null && !string.IsNullOrEmpty(workSheet.Cells[startRow, 1].Value.ToString()))
                        {
                            ...
                            {
                                PHF_BM_03TT_NSDP detail = new PHF_BM_03TT_NSDP()
                                {
                                    ObjectState = ObjectState.Added,
