...
{
    public interface INV_BM_12TT_NSDPService : IBaseService<PHF_BM_12TT_NSDP>
    {
        string UploadFileReport(HttpRequest request, string currentUser, string unitCode, string report, string period);
    }
    public class NV_BM_12TT_NSDPService : BaseService<PHF_BM_12TT_NSDP>, INV_BM_12TT_NSDPService
    {
        private readonly IRepositoryAsync<PHF_BM_12TT_NSDP> _repository;
        public NV_BM_12TT_NSDPService(IRepositoryAsync<PHF_BM_12TT_NSDP> repository) : base(repository)
        {
        ...
        public string UploadFileReport(HttpRequest request, string currentUser, string unitCode, string report, string period)
        {
            try
            {
                List<PHF_BM_12TT_NSDP> listInsert = new List<PHF_BM_12TT_NSDP>();
                ...
                using (var excelPackage = new ExcelPackage(HttpContext.Current.Request.Files[0].InputStream))
                {
                    ...
                    if (workSheet != null)
                    {
                        ...
                        if (workSheet.Cells[rowHeader, 9] != null)
                        {
                            ...
                            {
                                bmfile_ncdp.TEN_BIEUMAU = "Biểu số 12/TTTC-NSĐP";
                            }
                        }
                        int rowTitle = 3;
                        if (workSheet.Cells[rowTitle, 1] != null)
                        {
                            ...
                            {
                                bmfile_ncdp.TIEUDE_BIEUMAU = "TỔNG HỢP DỰ ÁN CÓ KHỐI LƯỢNG HOÀN THÀNH NGHIỆM THU CHƯA ĐƯỢC THANH TOÁN ";
                            }
                        }
                        int startRow = 9;
                        int stt = 1;
                        ...
                        while (workSheet.Cells[startRow, 1].Value != null && !string.IsNullOrEmpty(workSheet.Cells[startRow, 1].Value.ToString()))
                        {
                            ...
                            {
                                PHF_BM_12TT_NSDP detail = new PHF_BM_12TT_NSDP()
                                {
                                    ObjectState = ObjectState.Added,
                                ...
                                detail.DUAN = workSheet.Cells[startRow, 2] != null ? workSheet.Cells[startRow, 2].Value?.ToString() : null;
                                ...
                                detail.CHUDAUTU = workSheet.Cells[startRow, 3] != null ? workSheet.Cells[startRow, 3].Value?.ToString() : null;
                                detail.THOIDIEM_KHOICONG = workSheet.Cells[startRow, 4] != null ? workSheet.Cells[startRow, 4].Value?.ToString() : null;
                                detail.THOIGIAN_HOANTHANH = workSheet.Cells[startRow, 5] != null ? workSheet.Cells[startRow, 5].Value?.ToString() : null;
                                detail.GIATRI_HOPDONG = workSheet.Cells[startRow, 6] != null ? workSheet.Cells[startRow, 6].Value?.ToString() : null;
                                detail.GIATRI_NGHIEMTHU = workSheet.Cells[startRow, 7] != null ? workSheet.Cells[startRow, 7].Value?.ToString() : null;
                                detail.GIATRI_DUOC_THANHTOAN = workSheet.Cells[startRow, 8] != null ? workSheet.Cells[startRow, 8].Value?.ToString() : null;
                                detail.GIATRI_CHUA_THANHTOAN = workSheet.Cells[startRow, 9] != null ? workSheet.Cells[startRow, 9].Value?.ToString() : null;
                                detail.NGUYENNHAN = workSheet.Cells[startRow, 10] != null ? workSheet.Cells[startRow, 10].Value?.ToString() : null;
