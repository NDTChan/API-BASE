...
{
    public interface INV_BM_14TT_NSDPService : IBaseService<PHF_BM_14TT_NSDP>
    {
        string UploadFileReport(HttpRequest request, string currentUser, string unitCode, string report, string period);
    }
    public class NV_BM_14TT_NSDPService : BaseService<PHF_BM_14TT_NSDP>, INV_BM_14TT_NSDPService
    {
        private readonly IRepositoryAsync<PHF_BM_14TT_NSDP> _repository;
        public NV_BM_14TT_NSDPService(IRepositoryAsync<PHF_BM_14TT_NSDP> repository) : base(repository)
        {
        ...
        public string UploadFileReport(HttpRequest request, string currentUser, string unitCode, string report, string period)
        {
            try
            {
                List<PHF_BM_14TT_NSDP> listInsert = new List<PHF_BM_14TT_NSDP>();
                ...
                using (var excelPackage = new ExcelPackage(HttpContext.Current.Request.Files[0].InputStream))
                {
                    ...
                    if (workSheet != null)
                    {
                        ...
                        if (workSheet.Cells[rowHeader, 9] != null)
                        {
                            ...
                            {
                                bmfile_ncdp.TEN_BIEUMAU = "Biểu số 14/TTTC-NSĐP";
                            }
                        }
                        int rowTitle = 3;
                        if (workSheet.Cells[rowTitle, 2] != null)
                        {
                            if (!string.IsNullOrEmpty(workSheet.Cells[rowTitle, 2].Value?.ToString()))
                            {
                                bmfile_ncdp.TIEUDE_BIEUMAU = workSheet.Cells[rowTitle, 2].Value?.ToString();
                            }
                            else
                            {
                                bmfile_ncdp.TIEUDE_BIEUMAU = "TỔNG HỢP CÁC SAI PHẠM, KHUYẾT ĐIỂM QUA THANH TRA CÁC DỰ ÁN ĐẦU TƯ XÂY DỰNG";
                            }
                        }
                        int startRow = 9;
                        int stt = 1;
                        ...
                        while (workSheet.Cells[startRow, 1].Value != null && !string.IsNullOrEmpty(workSheet.Cells[startRow, 1].Value.ToString()))
                        {
                            ...
                            {
                                PHF_BM_14TT_NSDP detail = new PHF_BM_14TT_NSDP()
                                {
                                    ObjectState = ObjectState.Added,
                                ...
                                detail.TEN_DUAN = workSheet.Cells[startRow, 3] != null ? workSheet.Cells[startRow, 3].Value?.ToString() : null;
                                detail.DUAN_DAUTU = workSheet.Cells[startRow, 4] != null ? workSheet.Cells[startRow, 4].Value?.ToString() : null;
                                detail.DUAN_DUTOAN = workSheet.Cells[startRow, 5] != null ? workSheet.Cells[startRow, 5].Value?.ToString() : null;
                                detail.THOIGIAN_KHOICONG = workSheet.Cells[startRow, 6] != null ? workSheet.Cells[startRow, 6].Value?.ToString() : null;
                                detail.PHATHIEN_SAIPHAM = workSheet.Cells[startRow, 7] != null ? workSheet.Cells[startRow, 7].Value?.ToString() : null;
                                detail.PHATHIEN_SOTIEN = workSheet.Cells[startRow, 8] != null ? workSheet.Cells[startRow, 8].Value?.ToString() : null;
                                detail.KIENNGHI_NOIDUNG = workSheet.Cells[startRow, 9] != null ? workSheet.Cells[startRow, 9].Value?.ToString() : null;
                                detail.KIENNGHI_SOTIEN = workSheet.Cells[startRow, 10] != null ? workSheet.Cells[startRow, 10].Value?.ToString() : null;
