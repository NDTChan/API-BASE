...
{
    public interface INV_BM_05TT_NSDPService : IBaseService<PHF_BM_05TT_NSDP>
    {
        string UploadFileReport(HttpRequest request, string currentUser, string unitCode, string report, string period);
    }
    public class NV_BM_05TT_NSDPService : BaseService<PHF_BM_05TT_NSDP>, INV_BM_05TT_NSDPService
    {
        private readonly IRepositoryAsync<PHF_BM_05TT_NSDP> _repository;
        public NV_BM_05TT_NSDPService(IRepositoryAsync<PHF_BM_05TT_NSDP> repository) : base(repository)
        {
        ...
        public string UploadFileReport(HttpRequest request, string currentUser, string unitCode, string report, string period)
        {
            try
            {
                List<PHF_BM_05TT_NSDP> listInsert = new List<PHF_BM_05TT_NSDP>();
                ...
                using (var excelPackage = new ExcelPackage(HttpContext.Current.Request.Files[0].InputStream))
                {
                    ...
                    if (workSheet != null)
                    {
                        ...
                        if (workSheet.Cells[rowHeader, 10] != null)
                        {
                            ...
                            {
                                bmfile_ncdp.TEN_BIEUMAU = "Biểu số 05/TTTC-NSĐP";
                            }
                        }
                        int rowTitle = 3;
                        if (workSheet.Cells[rowTitle, 1] != null)
                        {
                            ...
                            {
                                bmfile_ncdp.TIEUDE_BIEUMAU = "TỔNG HỢP SAI PHẠM, KHUYẾT ĐIỂM TRONG BỐ TRÍ KẾ HOẠCH VỐN ĐẦU TƯ XÂY DỰNG CHO CÁC DỰ ÁN";
                            }
                        }
                        int startRow = 9;
                        int stt = 1;
                        ...
                        while (workSheet.Cells[startRow, 1].Value != null && !string.IsNullOrEmpty(workSheet.Cells[startRow, 1].Value.ToString()))
                        {
                            ...
                            {
                                PHF_BM_05TT_NSDP detail = new PHF_BM_05TT_NSDP()
                                {
                                    ObjectState = ObjectState.Added,
                                ...
                                detail.QUYETDINH_SO = workSheet.Cells[startRow, 3] != null ? workSheet.Cells[startRow, 3].Value?.ToString() : null;
                                detail.QUYETDINH_NGAY = workSheet.Cells[startRow, 4] != null ? workSheet.Cells[startRow, 4].Value?.ToString() : null;
                                detail.TONGMUC_DAUTU = workSheet.Cells[startRow, 5] != null ? workSheet.Cells[startRow, 5].Value?.ToString() : null;
                                detail.THOIGIAN_KC_HT = workSheet.Cells[startRow, 6] != null ? workSheet.Cells[startRow, 6].Value?.ToString() : null;
                                detail.VON_DUOCGIAO = workSheet.Cells[startRow, 7] != null ? workSheet.Cells[startRow, 7].Value?.ToString() : null;
                                detail.BOTRI_KEHOACH_VON = workSheet.Cells[startRow, 8] != null ? workSheet.Cells[startRow, 8].Value?.ToString() : null;
                                detail.THUTU_UUTIEN = workSheet.Cells[startRow, 9] != null ? workSheet.Cells[startRow, 9].Value?.ToString() : null;
                                detail.VON_KHONGHOPLY = workSheet.Cells[startRow, 10] != null ? workSheet.Cells[startRow, 10].Value?.ToString() : null;
                                detail.SAIPHAM_KHAC = workSheet.Cells[startRow, 11] != null ? workSheet.Cells[startRow, 11].Value?.ToString() : null;
