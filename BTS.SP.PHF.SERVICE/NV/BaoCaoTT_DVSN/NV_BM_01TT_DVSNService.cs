...
{
    public interface INV_BM_01TT_DVSNService : IBaseService<PHF_BM_01TT_DVSN>
    {
        string UploadFileReport(HttpRequest request, string currentUser, string unitCode, string report, string period);
    }

    public class NV_BM_01TT_DVSNService : BaseService<PHF_BM_01TT_DVSN>, INV_BM_01TT_DVSNService
    {
        private readonly IRepositoryAsync<PHF_BM_01TT_DVSN> _repository;

        public NV_BM_01TT_DVSNService(IRepositoryAsync<PHF_BM_01TT_DVSN> repository) : base(repository)
        {
        ...
        public string UploadFileReport(HttpRequest request, string currentUser, string unitCode, string report, string period)
        {
            try
            {
                List<PHF_BM_01TT_DVSN> listInsert = new List<PHF_BM_01TT_DVSN>();
                ...
                using (var excelPackage = new ExcelPackage(HttpContext.Current.Request.Files[0].InputStream))
                {
                    ...
                    if (workSheet != null)
                    {
                        ...
                        int rowHeader = 3;
                        if (workSheet.Cells[rowHeader, 7] != null)
                        {
                            if (!string.IsNullOrEmpty(workSheet.Cells[rowHeader, 7].Value?.ToString()))
                            {
                                bmfile_dvsn.TEN_BIEUMAU = workSheet.Cells[rowHeader, 7].Value?.ToString();
                            }
                            else
                            {
                                bmfile_dvsn.TEN_BIEUMAU = "Biểu số 01/TTr.ĐVSN";
                            }
                        }
                        ...
                        if (workSheet.Cells[rowTitle, 1] != null)
                        {
                            ...
                            {
                                bmfile_dvsn.TIEUDE_BIEUMAU = "TỔNG HỢP TÌNH HÌNH THỰC CHI CHI ĐƯỢC GIAO";
                            }
                        }
                        int startRow = 8;
                        int stt = 1;
                        ...
                        while (workSheet.Cells[startRow, 1].Value != null && !string.IsNullOrEmpty(workSheet.Cells[startRow, 1].Value.ToString()))
                        {
                            ...
                            {
                                PHF_BM_01TT_DVSN detail = new PHF_BM_01TT_DVSN()
                                {
                                    ObjectState = ObjectState.Added,
