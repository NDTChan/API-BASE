...
{
    public interface INV_BM_02TT_DVSNService : IBaseService<PHF_BM_02TT_DVSN>
    {
        string UploadFileReport(HttpRequest request, string currentUser, string unitCode, string report, string period);
    }

    public class NV_BM_02TT_DVSNService : BaseService<PHF_BM_02TT_DVSN>, INV_BM_02TT_DVSNService
    {
        private readonly IRepositoryAsync<PHF_BM_02TT_DVSN> _repository;

        public NV_BM_02TT_DVSNService(IRepositoryAsync<PHF_BM_02TT_DVSN> repository) : base(repository)
        {
        ...
        public string UploadFileReport(HttpRequest request, string currentUser, string unitCode, string report, string period)
        {
            try
            {
                List<PHF_BM_02TT_DVSN> listInsert = new List<PHF_BM_02TT_DVSN>();
                ...
                using (var excelPackage = new ExcelPackage(HttpContext.Current.Request.Files[0].InputStream))
                {
                    ...
                    if (workSheet != null)
                    {
                        ...
                        if (workSheet.Cells[rowHeader, 8] != null)
                        {
                            ...
                            {
                                bmfile_dvsn.TEN_BIEUMAU = "Biểu số 02/TTr.ĐVSN";
                            }
                        }
                        ...
                        if (workSheet.Cells[rowTitle, 1] != null)
                        {
                            ...
                            {
                                bmfile_dvsn.TIEUDE_BIEUMAU = "TỔNG HỢP THỰC CHI CHI GIAO KHÔNG ĐÚNG";
                            }
                        }
                        int startRow = 8;
                        int stt = 1;
                        ...
                        while (workSheet.Cells[startRow, 1].Value != null && !string.IsNullOrEmpty(workSheet.Cells[startRow, 1].Value.ToString()))
                        {
                            ...
                            {
                                PHF_BM_02TT_DVSN detail = new PHF_BM_02TT_DVSN()
                                {
                                    ObjectState = ObjectState.Added,
