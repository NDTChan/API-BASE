...
{
    public interface INV_BM_08TT_CQHCService : IBaseService<PHF_BM_08TT_CQHC>
    {
        ...
        DTO_BM_08_FILE_CQHC GetDataReport(string fileName, string fileNamePk);
    }
    public class NV_BM_08TT_CQHCService : BaseService<PHF_BM_08TT_CQHC>, INV_BM_08TT_CQHCService
    {
        private readonly IRepositoryAsync<PHF_BM_08TT_CQHC> _repository;
        public NV_BM_08TT_CQHCService(IRepositoryAsync<PHF_BM_08TT_CQHC> repository) : base(repository)
        {
        ...
        public string UploadFileReport(HttpRequest request, string currentUser, string unitCode, string report, string period, string doiTuong)
        {
            try
            {
                List<PHF_BM_08TT_CQHC> listInsert = new List<PHF_BM_08TT_CQHC>();
                ...
                using (var excelPackage = new ExcelPackage(HttpContext.Current.Request.Files[0].InputStream))
                {
                    ...
                    if (workSheet != null)
                    {
                        ...
                        if (workSheet.Cells[rowHeader, 6] != null)
                        {
                            ...
                            {
                                bmfile_ncdp.TEN_BIEUMAU = "Biểu số 08/TTr-CQHC";
                            }
                        }
                        ...
                        while (workSheet.Cells[startRow, 1].Value != null && !string.IsNullOrEmpty(workSheet.Cells[startRow, 1].Value.ToString()))
                        {
                            ...
                            {
                                PHF_BM_08TT_CQHC detail = new PHF_BM_08TT_CQHC()
                                {
                                    ObjectState = ObjectState.Added,
                                ...
                                detail.SLCDV_THUNHAP_CHIUTHUE = workSheet.Cells[startRow, 3] != null ? workSheet.Cells[startRow, 3].Value?.ToString() : null;
                                detail.SLCDV_SOTHUE_PHAINOP = workSheet.Cells[startRow, 4] != null ? workSheet.Cells[startRow, 4].Value?.ToString() : null;
                                detail.TTXD_THUNHAP_CHIUTHUE = workSheet.Cells[startRow, 5] != null ? workSheet.Cells[startRow,5].Value?.ToString() : null;
                                detail.TTXD_SOTHUE_PHAINOP = workSheet.Cells[startRow, 6] != null ? workSheet.Cells[startRow, 6].Value?.ToString() : null;
                                detail.CL_THUNHAP_CHIUTHUE = workSheet.Cells[startRow, 7] != null ? workSheet.Cells[startRow, 7].Value?.ToString() : null;
                                detail.CL_SOTHUE_PHAINOP = workSheet.Cells[startRow, 8] != null ? workSheet.Cells[startRow, 8].Value?.ToString() : null;
                                detail.NGUYENNHAN = workSheet.Cells[startRow, 9] != null ? workSheet.Cells[startRow, 9].Value?.ToString() : null;
        ...
        public DTO_BM_08_FILE_CQHC GetDataReport(string fileName, string fileNamePk)
        {
            DTO_BM_08_FILE_CQHC result = new DTO_BM_08_FILE_CQHC();
            if (!string.IsNullOrEmpty(fileName) && !string.IsNullOrEmpty(fileNamePk))
            {
                ...
                if (dto != null)
                {
                    ...
                    if (details.Count > 0)
                    {
                        ...
                        {
                            PHF_BM_08TT_CQHC_DTO row = new PHF_BM_08TT_CQHC_DTO();
                            row.ID = record.Id;
                            row.STT = record.STT;
                            ...
                            row.SLCDV_THUNHAP_CHIUTHUE = record.SLCDV_THUNHAP_CHIUTHUE;
                            row.SLCDV_SOTHUE_PHAINOP = record.SLCDV_SOTHUE_PHAINOP;
                            row.TTXD_THUNHAP_CHIUTHUE = record.TTXD_THUNHAP_CHIUTHUE;
                            row.TTXD_SOTHUE_PHAINOP = record.TTXD_SOTHUE_PHAINOP;
                            row.CL_THUNHAP_CHIUTHUE = record.CL_THUNHAP_CHIUTHUE;
                            row.CL_SOTHUE_PHAINOP = record.CL_SOTHUE_PHAINOP;
