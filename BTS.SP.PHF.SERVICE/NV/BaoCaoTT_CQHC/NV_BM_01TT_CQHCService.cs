...
{
    public interface INV_BM_01TT_CQHCService : IBaseService<PHF_BM_01TT_CQHC>
    {
        ...
        DTO_BM_01_FILE_CQHC GetDataReport(string fileName, string fileNamePk);
    }
    public class NV_BM_01TT_CQHCService : BaseService<PHF_BM_01TT_CQHC>, INV_BM_01TT_CQHCService
    {
        private readonly IRepositoryAsync<PHF_BM_01TT_CQHC> _repository;

        public NV_BM_01TT_CQHCService(IRepositoryAsync<PHF_BM_01TT_CQHC> repository) : base(repository)
        {
        ...
        public string UploadFileReport(HttpRequest request, string currentUser, string unitCode, string report, string period, string doiTuong)
        {
            try
            {
                List<PHF_BM_01TT_CQHC> listInsert = new List<PHF_BM_01TT_CQHC>();
                ...
                using (var excelPackage = new ExcelPackage(HttpContext.Current.Request.Files[0].InputStream))
                {
                    ...
                    if (workSheet != null)
                    {
                        ...
                        if (workSheet.Cells[rowHeader, 1] != null)
                        {
                            ...
                            {
                                bmfile_cqhc.TEN_BIEUMAU = "Biểu số 01/TTTC-CQHC";
                            }
                        }
                        ...
                        if (workSheet.Cells[rowTitle, 1] != null)
                        {
                            ...
                            {
                                bmfile_cqhc.TIEUDE_BIEUMAU = "BẢNG THỐNG KÊ VỀ TỔ CHỨC BỘ MÁY VÀ CHỨC NĂNG NHIỆM VỤ";
                            }
                        }
                        int startRow = 6;
                        int stt = 1;
                        ...
                        while (workSheet.Cells[startRow, 1].Value != null && !string.IsNullOrEmpty(workSheet.Cells[startRow, 1].Value.ToString()))
                        {
                            ...
                            {
                                PHF_BM_01TT_CQHC detail = new PHF_BM_01TT_CQHC()
                                {
                                    ObjectState = Repository.Pattern.Infrastructure.ObjectState.Added,
                                ...
                                detail.NAMTRUOC_LIENKE = workSheet.Cells[startRow, 3] != null ? workSheet.Cells[startRow, 3].Value?.ToString() : null;
                                detail.NAMLAP_DUTOAN = workSheet.Cells[startRow, 4] != null ? workSheet.Cells[startRow, 4].Value?.ToString() : null;
        ...
        public DTO_BM_01_FILE_CQHC GetDataReport(string fileName, string fileNamePk)
        {
            DTO_BM_01_FILE_CQHC result = new DTO_BM_01_FILE_CQHC();
            if (!string.IsNullOrEmpty(fileName) && !string.IsNullOrEmpty(fileNamePk))
            {
                ...
                if (dto != null)
                {
                    ...
                    if (details.Count > 0)
                    {
                        ...
                        {
                            PHF_BM_01TT_CQHC_DTO row = new PHF_BM_01TT_CQHC_DTO();
                            row.ID = record.Id;
                            row.STT = record.STT;
                            ...
                            row.NAMTRUOC_LIENKE = record.NAMTRUOC_LIENKE;
                            row.NAMLAP_DUTOAN = record.NAMLAP_DUTOAN;
