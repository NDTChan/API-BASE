...
--------------------------------------------------------
--------------------------------------------------------
--  DDL for Procedure PHB_BIEU11TT344_SR_XA_TM
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "BTSTC"."PHB_BIEU11TT344_SR_XA_TM" 
(
  MADBHC IN NVARCHAR2,
...
BEGIN
    ...
    END IF;
        ...
        P_QUERY:='SELECT bieub01xDetail.MA_CHUONG,bieub01xDetail.MA_LOAI,bieub01xDetail.MA_NGANHKT,bieub01xDetail.MA_MUC,bieub01xDetail.MA_TIEUMUC,bieub01xDetail.TEN_TIEUMUC AS DIEN_GIAI,SUM(bieub01xDetail.GIA_TRI_HACH_TOAN) as QUYET_TOAN
            FROM PHA_HACHTOAN_CHI bieub01xDetail WHERE NGAY_KET_SO >= '''||TUNGAY_KS||'''
                                                        AND NGAY_KET_SO <= '''||DENNGAY_KS||'''
                                                        AND NGAY_HIEU_LUC >= '''||TUNGAY_HL||'''
                                                        AND NGAY_HIEU_LUC <= '''||DENNGAY_HL||'''';
        P_QUERY:= P_QUERY || P_SELECT||P_WHERE;
        P_QUERY:=P_QUERY||' GROUP BY bieub01xDetail.MA_CHUONG,bieub01xDetail.MA_LOAI,bieub01xDetail.MA_NGANHKT,bieub01xDetail.TEN_TIEUMUC,bieub01xDetail.MA_MUC,bieub01xDetail.MA_TIEUMUC';
        P_QUERY:=P_QUERY||' ORDER BY bieub01xDetail.MA_CHUONG,bieub01xDetail.MA_LOAI,bieub01xDetail.MA_NGANHKT,bieub01xDetail.TEN_TIEUMUC,bieub01xDetail.MA_MUC,bieub01xDetail.MA_TIEUMUC';

--        P_QUERY:= 'SELECT * FROM PHA_HACHTOAN_CHI WHERE 1 = 2';
    DBMS_output.put_line(P_QUERY);
    OPEN cur FOR P_QUERY;
END PHB_BIEU11TT344_SR_XA_TM;
