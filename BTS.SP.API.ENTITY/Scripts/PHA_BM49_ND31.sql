create or replace PROCEDURE PHA_BM49_ND31 (P_LOAI_BAOCAO IN VARCHAR2, P_LOAI_DL IN VARCHAR2,MABAOCAO IN VARCHAR2,TABLENAME_TH IN VARCHAR2,P_TABLENAME IN VARCHAR2, TUNGAY_KS IN DATE, DENNGAY_KS IN DATE, TUNGAY_HL IN DATE, DENNGAY_HL IN DATE,P_CONGTHUC IN NVARCHAR2, DONVI_TIEN NUMBER, cur OUT SYS_REFCURSOR) IS
   ...
   V_CONGTHUC_DUTOAN_QUYETTOAN clob;
     ...
     -- TAO TABLE DE TONG HOP DU LIEU
        ...
        END IF;
        IF(P_CT IS NULL OR P_CT<=0) THEN P_CT:='1=1'; 
        ELSE P_CT:=P_CT;
        END IF;
  ...
  TABLESPACE "BTSTC"';
    ...
    FOR item_R IN (SELECT SAPXEP,STT,CAP,INDAM,INNGHIENG,HIENTHI,TRANG_THAI,LOAICHITIEU,MA_CHITIEU,TEN_CHITIEU,CONGTHUC,CONGTHUC_WHERE,CONGTHUC_DH_WHERE,ID FROM DM_CHITIEU_BAOCAO WHERE MA_BAOCAO = MABAOCAO ORDER BY SAPXEP ASC)
    LOOP
        ...
        BEGIN
                ...
                FOR item_C IN (SELECT MA_COT,CONGTHUC_WHERE,TRANG_THAI FROM DM_CHITIEU_BAOCAO_COL WHERE MA_BAOCAO=MABAOCAO ORDER BY STT ASC)
                    LOOP
                         BEGIN
                            SELECT GIA_TRI INTO GIA_TRI_CLO  FROM DM_CHITIEU_COT_SOLIEU  WHERE MA_COT=item_C.MA_COT AND MA_CHITIEU=item_R.MA_CHITIEU AND LOAI_CHITIEU=item_R.LOAICHITIEU AND NAM='2017';    
                            EXCEPTION WHEN OTHERS THEN GIA_TRI_CLO:=0;
                        END;
                        
                        IF(UPPER(P_LOAI_BAOCAO) = 'DH') THEN 
                        IF(item_C.TRANG_THAI='A') THEN V_CONGTHUC_DUTOAN_QUYETTOAN:=item_R.CONGTHUC_DH_WHERE;END IF;-- CONG THỨC CỦA ĐIỀU HÀNH
                        IF(item_C.TRANG_THAI='B')THEN V_CONGTHUC_DUTOAN_QUYETTOAN:=item_R.CONGTHUC; END IF;-- CONG THỨC CỦA DỰ TOÁN 
                        END IF;
                        IF(UPPER(P_LOAI_BAOCAO) = 'QT') THEN 
                        IF(item_C.TRANG_THAI='A') THEN V_CONGTHUC_DUTOAN_QUYETTOAN:=item_R.CONGTHUC_WHERE;END IF;-- CONG TH?C C?A QUY?T TOÁN
                        IF(item_C.TRANG_THAI='B')THEN V_CONGTHUC_DUTOAN_QUYETTOAN:=item_R.CONGTHUC; END IF;-- CONG TH?C C?A D? TOÁN   
                        END IF;
        ...
        BEGIN
                  ...
                  P_INSERT_C_R:=P_INSERT_C_R || 'VALUES('''|| item_R.SAPXEP ||''','''|| NVL(item_R.STT,'NULL') ||''','|| NVL(item_R.CAP,0) ||','|| NVL(item_R.INDAM,0) ||','|| NVL(item_R.INNGHIENG,0) ||','''|| item_R.HIENTHI ||''',''' || item_R.TRANG_THAI  ||''','|| NVL(item_R.LOAICHITIEU,0) ||','''  ||  item_R.CONGTHUC_WHERE ||  ''',''' ||  item_R.MA_CHITIEU || ''',''''''' ||  item_R.MA_CHITIEU || ''''''',';
                  P_INSERT_C_R:=P_INSERT_C_R || '''' || item_R.TEN_CHITIEU || '''' ||')';
         ...
         END;
        END IF;
        IF item_R.TRANG_THAI='CTMTQG' THEN
        BEGIN
                  P_INSERT_C_R:= 'INSERT INTO '|| P_TABLENAME ||'(SAPXEP,STT,CAP,INDAM,INNGHIENG,HIENTHI,TRANG_THAI,LOAI_CHITIEU,CT,MA_CHITIEU, MA_CHITIEU_2, TEN_CHITIEU)';
                  P_INSERT_C_R:=P_INSERT_C_R || 'VALUES('''|| item_R.SAPXEP ||''','''|| NVL(item_R.STT,'NULL') ||''','|| NVL(item_R.CAP,0) ||','|| NVL(item_R.INDAM,0) ||','|| NVL(item_R.INNGHIENG,0) ||','''|| item_R.HIENTHI ||''',''' || item_R.TRANG_THAI  ||''','|| NVL(item_R.LOAICHITIEU,0) ||','''  || '-' ||  ''',''' ||  item_R.MA_CHITIEU || ''',''''''' ||  item_R.MA_CHITIEU || ''''''',';
                  P_INSERT_C_R:=P_INSERT_C_R || '''' || item_R.TEN_CHITIEU || '''' ||')';
                  EXECUTE IMMEDIATE P_INSERT_C_R;
        END;
        END IF;
