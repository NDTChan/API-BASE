create or replace PROCEDURE PHA_BCCT_UOC(P_CAP IN NVARCHAR2, P_USER_NAME IN NVARCHAR2, P_MA_DBHC IN NVARCHAR2,P_LOAI_BAOCAO IN VARCHAR2,P_LOAI_DL IN VARCHAR2,MABAOCAO IN VARCHAR2,TABLENAME_TH IN VARCHAR2,P_TABLENAME IN VARCHAR2, TUNGAY_KS IN DATE, DENNGAY_KS IN DATE, TUNGAY_HL IN DATE, DENNGAY_HL IN DATE,P_CONGTHUC IN NVARCHAR2, DONVI_TIEN NUMBER, cur OUT SYS_REFCURSOR) IS
    -- Báo cáo UOC -
   ...
   V_TU_NAM_HL NUMBER := TO_NUMBER(to_char(TUNGAY_HL,'yyyy'));
   V_TU_NAM_KS NUMBER := TO_NUMBER(to_char(TUNGAY_KS,'yyyy'));
    ...
    LOOP      
        ...
        BEGIN
                ...
                IF(item_C.MA_COT = 'TH') THEN
                 ...
                 P_CT:=P_CT || P_WHERE_DATE_HL_TU ||  P_WHERE_DATE_HL_DEN || P_WHERE_DATE_KS_TU ||  P_WHERE_DATE_KS_DEN;
                 END IF;
                 ELSIF(item_C.MA_COT = 'LK_TH') THEN
                 P_WHERE_DATE_HL_TU:= ' AND NGAY_HIEU_LUC >= TO_DATE ('''|| '0101'||V_TU_NAM_HL ||''', ''ddMMyyyy'')';
                 P_WHERE_DATE_HL_DEN:= ' AND NGAY_HIEU_LUC <= TO_DATE ('''||to_char(DENNGAY_HL,'ddMMyyyy')||''', ''ddMMyyyy'')';        
                 P_WHERE_DATE_KS_TU:= ' AND NGAY_KET_SO >= TO_DATE ('''|| '0101'||V_TU_NAM_KS ||''', ''ddMMyyyy'')'; 
                 P_WHERE_DATE_KS_DEN:= ' AND NGAY_KET_SO <= TO_DATE ('''||to_char(DENNGAY_KS,'ddMMyyyy')||''', ''ddMMyyyy'')';
                 P_SELECT_DBHC:= ' AND MA_DIABAN IN (SELECT MA_DBHC FROM DM_DBHC WHERE MA_DBHC IN (SELECT MA_DBHC FROM DM_DBHC WHERE MA_DBHC = '''||P_MA_DBHC||''' OR MA_DBHC_CHA = '''||P_MA_DBHC||''') OR MA_DBHC_CHA IN (SELECT MA_DBHC FROM DM_DBHC WHERE MA_DBHC = '''||P_MA_DBHC||''' OR MA_DBHC_CHA = '''||P_MA_DBHC||'''))';
                 IF(P_SELECT_DBHC IS NOT NULL) THEN
                 P_CT:=P_CT || P_WHERE_DATE_HL_TU ||  P_WHERE_DATE_HL_DEN || P_WHERE_DATE_KS_TU ||  P_WHERE_DATE_KS_DEN || P_SELECT_DBHC;
                 ...
                 END IF;
                         ...
                         SELECT SUM(NVL(GIA_TRI,0)) INTO GIA_TRI_CLO  FROM DM_CHITIEU_COT_SOLIEU  WHERE 
                            MA_COT=item_C.MA_COT AND MA_CHITIEU=item_R.MA_CHITIEU AND LOAI_CHITIEU=item_R.LOAICHITIEU AND NAM= V_TU_NAM_HL 
...
-- B2 UPDATE CONG THUC TONG TAI CAC CHI TIEU CO TRANG THAI S 
    P_UPDATE_SUM:=''; P_SUM_U:='';
     ...
     FOR item_C IN (SELECT MA_COT,CONGTHUC_WHERE FROM DM_CHITIEU_BAOCAO_COL WHERE MA_BAOCAO=MABAOCAO AND MA_DBHC LIKE (SELECT MA_T FROM DM_DBHC WHERE MA_DBHC LIKE ''||P_MA_DBHC||'')  ORDER BY STT ASC )
            LOOP   
    ...
    P_SELECT_2 := 'SELECT SUBSTR(ct,instr(trim(ct),'','') + 1, length(trim(ct)) - instr(trim(ct),'','') - 1 ) FROM '|| P_TABLENAME ||' WHERE TRANG_THAI =''T4'' AND USERNAME = '''|| P_USER_NAME ||'''';
     ...
     FOR item_C IN (SELECT MA_COT,CONGTHUC_WHERE FROM DM_CHITIEU_BAOCAO_COL WHERE MA_BAOCAO=MABAOCAO AND MA_DBHC LIKE (SELECT MA_T FROM DM_DBHC WHERE MA_DBHC LIKE ''||P_MA_DBHC||'') ORDER BY STT ASC )
      ...
      EXECUTE IMMEDIATE P_UPDATE_SUBTRACT; 
        ...
        --- B2 UPDATE CONG THUC TONG TAI CAC CHI TIEU CO TRANG THAI S 6(khác loại chỉ tiêu)
	   P_UPDATE_SUM:=''; P_SUM_U:='';
     P_UPDATE_SUM:=' UPDATE ' || P_TABLENAME || ' A SET ';
     FOR item_C IN (SELECT MA_COT,CONGTHUC_WHERE FROM DM_CHITIEU_BAOCAO_COL WHERE MA_BAOCAO=MABAOCAO AND MA_DBHC LIKE (SELECT MA_T FROM DM_DBHC WHERE MA_DBHC LIKE ''||P_MA_DBHC||'') ORDER BY STT ASC )
            LOOP   
                  P_SUM_U:=P_SUM_U || 'A.'|| item_C.MA_COT || ' = (SELECT NVL(SUM('|| item_C.MA_COT ||'),0) FROM '||P_TABLENAME||' B WHERE instr(A.CT,B.MA_CHITIEU_2)>0 AND A.USERNAME = B.USERNAME),';                  
            END LOOP;
       P_SUM_U:= SUBSTR(P_SUM_U,0,LENGTh(P_SUM_U)-1);
