create or replace PROCEDURE PHA_BCCT_R_C_V7(P_CAP IN NVARCHAR2, P_USER_NAME IN NVARCHAR2, P_MA_DBHC IN NVARCHAR2,P_LOAI_DL IN VARCHAR2,MABAOCAO IN VARCHAR2,TABLENAME_TH IN VARCHAR2,P_TABLENAME IN VARCHAR2, TUNGAY_KS IN DATE, DENNGAY_KS IN DATE, TUNGAY_HL IN DATE, DENNGAY_HL IN DATE,P_CONGTHUC IN NVARCHAR2, DONVI_TIEN NUMBER, cur OUT SYS_REFCURSOR) IS
    -- Báo cáo B201,B301 -
  ...
  /*P_SQL_CREATE_TABLE := 'CREATE GLOBAL TEMPORARY TABLE  '||P_TABLENAME||'(SAPXEP  NVARCHAR2(10) , CT NVARCHAR2(2000),CAP NUMBER(10,0), INDAM NUMBER(10,0), INNGHIENG NUMBER(10,0),HIENTHI NVARCHAR2(1), TRANG_THAI NVARCHAR2(10),LOAI_CHITIEU NUMBER(10,0),MA_CHITIEU NVARCHAR2(50),MA_CHITIEU_2 NVARCHAR2(50),TEN_CHITIEU NVARCHAR2(500),' 
    ...
    LOOP      
        ...
        BEGIN
                ...
                IF(P_CONGTHUC IS NULL OR P_CT = '') THEN P_CT:='1=1'; 
                END IF;
                IF(item_C.MA_COT = 'PS_NSTW' OR item_C.MA_COT= 'PS_NSTT' OR item_C.MA_COT = 'PS_NSQH' OR item_C.MA_COT = 'PS_NSPX' OR item_C.MA_COT ='PSTW' OR item_C.MA_COT='PSTT' OR item_C.MA_COT='PSQH' OR item_C.MA_COT='PSPX') THEN
                 P_WHERE_DATE_HL_TU:= ' AND NGAY_HIEU_LUC >= TO_DATE ('''||to_char(TUNGAY_HL,'ddMMyyyy')||''', ''ddMMyyyy'')';
                 P_WHERE_DATE_HL_DEN:= ' AND NGAY_HIEU_LUC <= TO_DATE ('''||to_char(DENNGAY_HL,'ddMMyyyy')||''', ''ddMMyyyy'')';        
                 P_WHERE_DATE_KS_TU:= ' AND NGAY_KET_SO >= TO_DATE ('''||to_char(TUNGAY_KS,'ddMMyyyy')||''', ''ddMMyyyy'')'; 
                 P_WHERE_DATE_KS_DEN:= ' AND NGAY_KET_SO <= TO_DATE ('''||to_char(DENNGAY_KS,'ddMMyyyy')||''', ''ddMMyyyy'')';
                 ...
                 P_CT:=P_CT || P_WHERE_DATE_HL_TU ||  P_WHERE_DATE_HL_DEN || P_WHERE_DATE_KS_TU ||  P_WHERE_DATE_KS_DEN;
                 END IF;
                 ELSIF(item_C.MA_COT = 'LK_NSTW' OR item_C.MA_COT= 'LK_NSTT' OR item_C.MA_COT = 'LK_NSQH' OR item_C.MA_COT = 'LK_NSPX' OR item_C.MA_COT ='LKTW' OR item_C.MA_COT='LKTT' OR item_C.MA_COT='LKQH' OR item_C.MA_COT='LKPX') THEN
