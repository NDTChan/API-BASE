create or replace PROCEDURE PHA_BCCT_R_C_V8 (P_CAP IN NVARCHAR2, P_USER_NAME IN NVARCHAR2,P_MA_DBHC IN NVARCHAR2,P_LOAI_BAOCAO IN VARCHAR2, P_LOAI_DL IN VARCHAR2,MABAOCAO IN VARCHAR2,TABLENAME_TH IN VARCHAR2,P_TABLENAME IN VARCHAR2, TUNGAY_KS IN DATE, DENNGAY_KS IN DATE, TUNGAY_HL IN DATE, DENNGAY_HL IN DATE,P_CONGTHUC IN NVARCHAR2, DONVI_TIEN NUMBER, cur OUT SYS_REFCURSOR) IS
  ...
  ';                      
       ...
       -- table PHA_HACHTOAN_THU              
        ...
        -- table PHA_DUTOAN
        TABLE_PHA_DT_DUTOAN:='PHA_HACHTOAN_KHAC';
        SQL_QUERRY := 'SELECT 3,MA_TKTN ,MA_DIABAN,MA_CAP,MA_CHUONG,MA_NGANHKT,MA_LOAI,MA_TIEUMUC,MA_MUC,MA_TIEUNHOM,substr(MA_CTMTQG,2,length(MA_CTMTQG)),MA_KBNN,MA_NGUON_NSNN,MA_DVQHNS,MA_NGHIEPVU,TO_DATE(to_char(NGAY_HIEU_LUC,''ddMMyyyy''), ''ddMMyyyy''),TO_DATE (to_char(NGAY_KET_SO,''ddMMyyyy''), ''ddMMyyyy''),ATTRIBUTE8
        ,SUM(ENTERED_DR-ENTERED_CR) FROM '||TABLE_PHA_DT_DUTOAN||' VC WHERE MA_TKTN IN (''1724'',''1727'',''1971'',''1764'',''1767'',''1598'',''1972'')
         GROUP BY  MA_TKTN,MA_DIABAN,MA_CAP,MA_CHUONG,MA_NGANHKT,MA_LOAI,MA_TIEUMUC,MA_MUC,MA_TIEUNHOM,MA_CTMTQG,MA_KBNN,MA_NGUON_NSNN,MA_DVQHNS,MA_NGHIEPVU,NGAY_HIEU_LUC,NGAY_KET_SO,ATTRIBUTE8';   
  ...
  /*P_SQL_CREATE_TABLE := 'CREATE GLOBAL TEMPORARY TABLE  '||P_TABLENAME||'(SAPXEP  NVARCHAR2(10) , CT NVARCHAR2(2000),CAP NUMBER(10,0), INDAM NUMBER(10,0), INNGHIENG NUMBER(10,0),HIENTHI NVARCHAR2(1), TRANG_THAI NVARCHAR2(10),LOAI_CHITIEU NUMBER(10,0),MA_CHITIEU NVARCHAR2(50),MA_CHITIEU_2 NVARCHAR2(50),TEN_CHITIEU NVARCHAR2(500),' 
    ...
    LOOP      
        ...
        BEGIN
                ...
                FOR item_C IN (SELECT MA_COT,CONGTHUC_WHERE,CONGTHUC_DH_WHERE,CONGTHUC_DH,TRANG_THAI FROM DM_CHITIEU_BAOCAO_COL WHERE MA_BAOCAO=MABAOCAO AND MA_DBHC LIKE (SELECT MA_T FROM DM_DBHC WHERE MA_DBHC LIKE ''||P_MA_DBHC||'') ORDER BY STT ASC)
                    LOOP
                         BEGIN
                            IF P_CAP = '4' THEN
...
--                                 END IF;
            ...
            BEGIN
                  ...
                  LOOP
                    ...
                    N_COUNT_2:=N_COUNT_2+1;
                      ...
                      P_INSERT_C_R:=P_INSERT_C_R || 'VALUES('''|| P_USER_NAME ||''','||N_COUNT_2||','||N_COUNT||',0,0,0,1,''CTMTQG_C'',0,''' || item_CTMT.MA_CTMTQG ||''',';
        ...
        END IF;
                     ...
                     P_UPDATE_SUBTRACT:='';
                      P_UPDATE_SUBTRACT:=' UPDATE ' || P_TABLENAME || ' A SET A.TONG = (A.NST + A.NSH + A.NSX) WHERE A.MA_CHITIEU = ''65800'' AND USERNAME = '''|| P_USER_NAME ||'''';
...
-- B2 UPDATE CONG THUC TONG TAI CAC CHI TIEU CO TRANG THAI S 1
   ...
   -- DBMS_OUTPUT.put_line ('T5:=' ||  P_UPDATE_SUM );
    ...
    P_SELECT_1 := 'SELECT SUBSTR(ct,2,INSTR(trim(ct),'','') - 2) FROM '|| P_TABLENAME ||' WHERE TRANG_THAI =''T'' AND A.USERNAME = '''|| P_USER_NAME ||''' AND MA_CHITIEU_2 IN (MA_CHITIEU_2) ';
    P_SELECT_2 := 'SELECT SUBSTR(ct,instr(trim(ct),'','') + 1, length(trim(ct)) - instr(trim(ct),'','') - 1 ) FROM '|| P_TABLENAME ||' WHERE TRANG_THAI =''T'' AND A.USERNAME = '''|| P_USER_NAME ||''' ';
  ...
  --- B2 UPDATE CONG THUC TONG TAI CAC CHI TIEU CO TRANG THAI S 2
    ...
    --DBMS_OUTPUT.put_line ('T5:=' ||  P_SELECT_2 );
     ...
     FOR item_C IN (SELECT MA_COT,CONGTHUC_WHERE FROM DM_CHITIEU_BAOCAO_COL WHERE MA_BAOCAO=MABAOCAO  ORDER BY STT ASC )
            ...
            END LOOP;
       P_SUM_U:= SUBSTR(P_SUM_U,0,LENGTh(P_SUM_U)-1);
       P_UPDATE_SUM:=P_UPDATE_SUM || P_SUM_U ||' WHERE A.TRANG_THAI=''S6'' AND A.USERNAME = '''|| P_USER_NAME ||'''';
      -- DBMS_OUTPUT.put_line ('T7:=' ||  P_UPDATE_SUM );
      EXECUTE IMMEDIATE P_UPDATE_SUM; 
    ...
    --DBMS_OUTPUT.put_line ('T5:=' ||  P_SELECT_2 );
     ...
     FOR item_C IN (SELECT MA_COT,CONGTHUC_WHERE,CONGTHUC_DH_WHERE FROM DM_CHITIEU_BAOCAO_COL WHERE MA_BAOCAO=MABAOCAO ORDER BY STT ASC )
      ...
      EXECUTE IMMEDIATE P_UPDATE_SUBTRACT; 
      P_UPDATE_SUBTRACT:='';
       P_UPDATE_SUBTRACT:=' UPDATE ' || P_TABLENAME || ' A SET A.TONG = (A.NST + A.NSH + A.NSX) WHERE A.TRANG_THAI = ''T5'' AND A.USERNAME = '''|| P_USER_NAME ||'''';
       EXECUTE IMMEDIATE P_UPDATE_SUBTRACT;
    ...
    P_SELECT_2 := 'SELECT SUBSTR(ct,instr(trim(ct),'','') + 1, length(trim(ct)) - instr(trim(ct),'','') - 1 ) FROM '|| P_TABLENAME ||' WHERE TRANG_THAI =''T8'' AND USERNAME = '''|| P_USER_NAME ||'''';
     ...
     FOR item_C IN (SELECT MA_COT,CONGTHUC_WHERE FROM DM_CHITIEU_BAOCAO_COL WHERE MA_BAOCAO=MABAOCAO  ORDER BY STT ASC )
            ...
            END LOOP;
       P_SUM_U:= SUBSTR(P_SUM_U,0,LENGTh(P_SUM_U)-1);
       P_UPDATE_SUM:=P_UPDATE_SUM || P_SUM_U ||' WHERE A.TRANG_THAI=''S9'' AND A.USERNAME = '''|| P_USER_NAME ||'''';
      -- DBMS_OUTPUT.put_line ('T7:=' ||  P_UPDATE_SUM );
      EXECUTE IMMEDIATE P_UPDATE_SUM; 
     ...
     FOR item_C IN (SELECT MA_COT,CONGTHUC_WHERE FROM DM_CHITIEU_BAOCAO_COL WHERE MA_BAOCAO=MABAOCAO  ORDER BY STT ASC )
            ...
            END LOOP;
       P_SUM_U:= SUBSTR(P_SUM_U,0,LENGTh(P_SUM_U)-1);
       P_UPDATE_SUM:=P_UPDATE_SUM || P_SUM_U ||' WHERE A.TRANG_THAI=''S10'' AND A.USERNAME = '''|| P_USER_NAME ||'''';
      -- DBMS_OUTPUT.put_line ('T7:=' ||  P_UPDATE_SUM );
      EXECUTE IMMEDIATE P_UPDATE_SUM; 
    ...
    --B2 UPDATE CONG THUC tru TAI CAC CHI TIEU CO TRANG THAI T10(khác loại chỉ tiêu)
     P_UPDATE_SUBTRACT:=''; P_SUBTRACT_U:='';
    P_SELECT_1 := 'SELECT SUBSTR(ct,2,INSTR(trim(ct),'','') - 2) FROM '|| P_TABLENAME ||' WHERE TRANG_THAI =''T10'' AND USERNAME = '''|| P_USER_NAME ||''' AND MA_CHITIEU_2 IN (MA_CHITIEU_2) ';
    P_SELECT_2 := 'SELECT SUBSTR(ct,instr(trim(ct),'','') + 1, length(trim(ct)) - instr(trim(ct),'','') - 1 ) FROM '|| P_TABLENAME ||' WHERE TRANG_THAI =''T10'' AND USERNAME = '''|| P_USER_NAME ||''' ';
     P_UPDATE_SUBTRACT:=' UPDATE ' || P_TABLENAME || ' A SET ';
     FOR item_C IN (SELECT MA_COT,CONGTHUC_WHERE FROM DM_CHITIEU_BAOCAO_COL WHERE MA_BAOCAO=MABAOCAO  ORDER BY STT ASC )
            LOOP   
                  P_SUBTRACT_U:=P_SUBTRACT_U || 'A.'|| item_C.MA_COT || ' = ((SELECT NVL(('|| item_C.MA_COT ||'),0) FROM '||P_TABLENAME||' B WHERE B.MA_CHITIEU_2 = ('||P_SELECT_1||') AND A.USERNAME = B.USERNAME)
            ...
            END LOOP;
       P_SUBTRACT_U:= SUBSTR(P_SUBTRACT_U,0,LENGTh(P_SUBTRACT_U)-1);
       P_UPDATE_SUBTRACT:=P_UPDATE_SUBTRACT || P_SUBTRACT_U ||' WHERE A.TRANG_THAI=''T10'' AND A.USERNAME = '''|| P_USER_NAME ||'''';
       --DBMS_OUTPUT.put_line ('T5:=' ||  P_UPDATE_SUBTRACT );
      EXECUTE IMMEDIATE P_UPDATE_SUBTRACT;
