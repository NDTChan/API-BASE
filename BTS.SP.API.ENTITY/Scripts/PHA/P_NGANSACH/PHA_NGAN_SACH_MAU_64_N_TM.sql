create or replace PROCEDURE "PHA_NGAN_SACH_MAU_64_N_TM" (P_MA_DBHC IN NVARCHAR2,P_CONGTHUC VARCHAR2,TUNGAY_KS IN DATE, DENNGAY_KS IN DATE, TUNGAY_HL IN DATE, DENNGAY_HL IN DATE,DONVI_TIEN number, cur OUT SYS_REFCURSOR) AS 
...
BEGIN
    ...
    WHERE A.NGAY_HIEU_LUC >= TO_DATE ('''||to_char(TUNGAY_HL,'ddMMyyyy')||''', ''ddMMyyyy'')
                        ...
                        '||P_SQL_INSERT||'
    GROUP BY A.MA_NHOM,A.MA_TIEUNHOM,A.TEN_NHOM,A.TEN_TIEUNHOM, A.MA_MUC, A.MA_TIEUMUC, A.TEN_MUC, A.TEN_TIEUMUC
    ORDER BY A.MA_NHOM';
