create or replace PROCEDURE "PHA_NGAN_SACH_MAU_63" (P_MA_DBHC IN NVARCHAR2,P_CONGTHUC VARCHAR2, TUNGAY_KS IN DATE, DENNGAY_KS IN DATE, TUNGAY_HL IN DATE, DENNGAY_HL IN DATE,DONVI_TIEN number, cur OUT SYS_REFCURSOR) as
...
QUERY_STR:='select * FROM (SELECT 
          MA_CHUONG, TEN_CHUONG, MA_CAP,MA_MUC, TEN_MUC,MA_TIEUMUC,TEN_TIEUMUC, 
         MA_CAPMLNS,MA_LOAI, MA_KBNN, MA_DVQHNS, MA_NGUON_NSNN, MA_NGANHKT,
        ...
        'AND NGAY_HIEU_LUC >= TO_DATE ('''||to_char(TUNGAY_HL,'ddMMyyyy')||''', ''ddMMyyyy'')
         ...
         GROUP BY MA_CHUONG,TEN_CHUONG, MA_CAP, MA_CAPMLNS, MA_MUC,TEN_MUC, MA_TIEUMUC,TEN_TIEUMUC,MA_LOAI,MA_NGANHKT,MA_KBNN,MA_DVQHNS,MA_NGUON_NSNN
        )
        PIVOT ( Sum(LK) as LK
                  FOR MA_CAP
                  IN (''1'' AS TW, ''2'' AS TINH, ''3'' AS HUYEN, ''4'' AS XA)
                  )
        order by  MA_CHUONG DESC'; 
