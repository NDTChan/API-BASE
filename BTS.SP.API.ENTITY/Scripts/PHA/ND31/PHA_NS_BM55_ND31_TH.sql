create or replace PROCEDURE "PHA_NS_BM55_ND31_TH" (P_MA_DBHC IN NVARCHAR2,P_LOAI_BAOCAO IN VARCHAR2, P_CONGTHUC VARCHAR2, TUNGAY_KS IN DATE, DENNGAY_KS IN DATE, TUNGAY_HL IN DATE, DENNGAY_HL IN DATE, P_CAP VARCHAR2, DONVI_TIEN number, cur OUT SYS_REFCURSOR) as
   QUERY_STR  CLOB; 
   INSERT_STR  CLOB; 
   INSERT_STR1  CLOB;
   QUERY_STR_CHI CLOB;
   ...
   BEGIN
    ...
    IF(UPPER(P_LOAI_BAOCAO) = 'DH') THEN 
         ...
         select REPLACE(CONGTHUC_DH_WHERE, '''', '''') INTO CT_CDT_HDKT_GT from DM_CHITIEU_BAOCAO_COL WHERE MA_BAOCAO='CHITIEU_CHI' AND MA_COT='CDT_HDKT_GT';
         select REPLACE(CONGTHUC_DH_WHERE, '''', '''') INTO CT_CDT_HDKT_NN from DM_CHITIEU_BAOCAO_COL WHERE MA_BAOCAO='CHITIEU_CHI' AND MA_COT='CDT_HDKT_NN';
    ...
    END CASE;
        ...
        END IF;
            QUERY_STR:='SELECT HT.MA_CHUONG, HT.TEN_CHUONG
                        ...
                        FROM
                        ( SELECT QT.MA_CHUONG, QT.TEN_CHUONG
                        ...
                        and A.NGAY_KET_SO >=TO_DATE ('''||to_char(TUNGAY_KS,'ddMMyyyy')||''', ''ddMMyyyy'')
                         ...
                         GROUP BY A.MA_CHUONG,A.TEN_CHUONG, A.MA_DVQHNS, A.TEN_DVQHNS) DT ON QT.MA_CHUONG = DT.MA_CHUONG AND QT.MA_DVQHNS = DT.MA_DVQHNS
                            GROUP BY QT.MA_CHUONG, QT.TEN_CHUONG
