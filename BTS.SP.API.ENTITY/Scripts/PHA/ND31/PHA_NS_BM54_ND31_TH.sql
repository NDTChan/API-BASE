create or replace PROCEDURE PHA_NS_BM54_ND31_TH (P_MA_DBHC IN NVARCHAR2,P_LOAI_BAOCAO IN VARCHAR2, P_CONGTHUC VARCHAR2, TUNGAY_KS IN DATE, DENNGAY_KS IN DATE, TUNGAY_HL IN DATE, DENNGAY_HL IN DATE, P_CAP VARCHAR2, DONVI_TIEN number, cur OUT SYS_REFCURSOR) as
   QUERY_STR  CLOB; 
   INSERT_STR  CLOB; 
   INSERT_STR1  CLOB;
   QUERY_STR_CHI CLOB;
    ...
    IF(P_MA_DBHC <> 27) THEN
        ...
        END IF;
            ...
            select Cast(''1'' as nvarchar2(50)) as MA_CHUONG,Cast(''CÁC CƠ QUAN, TỔ CHỨC'' as nvarchar2(50)) as TEN_CHUONG
                        ...
                        , sum(0) as CDT_GD, sum(0) as CDT_YT, sum(0) as CDT_TDTT, sum(0) as CDT_PT, sum(0) as CDT_HTV , sum(0) as CDT_K
                        , sum(0) as DT_CDT_QP, sum(0) as DT_CDT_AN, sum(0) as DT_CDT_KH, sum(0) as DT_CDT_HDKT, sum(0) as DT_CDT_QLNN
                        , sum(0) as DT_CDT_BVMT, sum(0) as DT_CDT_BDXH, sum(0) as DT_CDT_VH, sum(0) as DT_CDT_KHCN
                        , sum(0) as DT_CDT_GD, sum(0) as DT_CDT_YT, sum(0) as DT_CDT_TDTT, sum(0) as DT_CDT_PT, sum(0) as DT_CDT_HTV , sum(0) as DT_CDT_K
                        , sum(0) as DT_CDT
                        ...
                        , sum(0) as  DT_CTX_QP, sum(0) as  DT_CTX_AN, sum(0) as DT_CTX_KH, sum(0) as DT_CTX_HDKT, sum(0) as DT_CTX_QLNN
                        , sum(0) as DT_CTX_BVMT, sum(0) as DT_CTX_BDXH, sum(0) as DT_CTX_VH, sum(0) as DT_CTX_KHCN
                        , sum(0) as DT_CTX_GD, sum(0) as DT_CTX_YT, sum(0) as DT_CTX_TDTT, sum(0) as DT_CTX_PT
                        , sum(0) as DT_CTX
                        , sum(0) as CTMTQG_DT, sum(0) as CTMTQG_TX
                        , sum(0) as DT_CTMTCDT, sum(0) as DT_CTMTCTX
                        , sum(0) as CTN, sum(0) as CBS, sum(0) as CCN
                        from dual
            )
            union all
            select * from (
            SELECT HT.MA_CHUONG, HT.TEN_CHUONG
                        ...
                        , NVL(HT.CTMTQG_DT,0) as CTMTQG_DT, NVL(HT.CTMTQG_TX,0) as CTMTQG_TX
                        , NVL(HT.DT_CTMTCDT,0) as DT_CTMTCDT, NVL(HT.DT_CTMTCTX,0) as DT_CTMTCTX
                        , NVL(HT.CTN,0) as CTN, NVL(HT.CBS,0) as CBS, NVL(HT.CCN,0) as CCN
                        ...
                        ,NVL(SUM (CASE WHEN ('|| CT_CTMTCDT ||')    THEN A.GIA_TRI_HACH_TOAN/NVL('|| DONVI_TIEN ||',1) ELSE 0 END),0) AS CTMTQG_DT
                        ,NVL(SUM (CASE WHEN ('|| CT_CTMTCTX ||')        THEN A.GIA_TRI_HACH_TOAN/NVL('|| DONVI_TIEN ||',1) ELSE 0 END),0) AS CTMTQG_TX
                        ...
                        AND NOT A.MA_CAP like ''1'' AND NOT A.MA_CHUONG LIKE ''000''  AND A.MA_CAPMLNS = '''||P_CAP||''''
                        ...
                        select Cast(''II'' as nvarchar2(50)) as MA_CHUONG, Cast(''CHI TRẢ NỢ LÃI DO CHÍNH QUYỀN ĐỊA PHƯƠNG VAY (2)'' as nvarchar2(50)) as TEN_CHUONG
                            ...
                            , sum(0) as CTMTQG_DT, sum(0) as CTMTQG_TX
                            , sum(0) as DT_CTMTCDT, sum(0) as DT_CTMTCTX
                            , SUM (CASE WHEN ((( MA_TKTN like ''8941'' OR  MA_TKTN like ''8951'' OR  MA_TKTN like ''8992'')  AND  (( MA_MUC like ''8300'' OR  MA_MUC like ''8350'') OR ( MA_TIEUMUC like ''8553'' OR  MA_TIEUMUC like ''8599''))  AND  ( MA_CHUONG like ''160'' OR  MA_CHUONG like ''560'')  AND   MA_NGANHKT like ''345'') OR  ( MA_TKTN like ''8941''  AND  ( MA_MUC like ''8400'' OR  MA_MUC like ''8450'' OR  MA_MUC like ''8500'' OR  MA_MUC like ''8550'')  AND  ( MA_CHUONG like ''160'' OR  MA_CHUONG like ''560'')  AND   MA_NGANHKT like ''348'')) THEN GIA_TRI_HACH_TOAN/NVL('|| DONVI_TIEN ||',1) ELSE 0 END) AS CTN
                            , sum(0) as CBS, sum(0) as CCN
                        from BM54_ND31 where MA_CAP <> 1 AND NGAY_HIEU_LUC >= TO_DATE ('''||to_char(TUNGAY_HL,'ddMMyyyy')||''', ''ddMMyyyy'')
                        ...
                        AND NOT MA_DVQHNS LIKE ''7%'' AND MA_CAPMLNS = '''||P_CAP||'''
                        ...
                        select Cast(''III'' as nvarchar2(50)) as MA_CHUONG, Cast(''CHI BỔ SUNG QUỸ DỰ TRỮ TÀI CHÍNH (2)'' as nvarchar2(50)) as TEN_CHUONG
                            ...
                            , sum(0) as DT_CTX_GD, sum(0) as DT_CTX_YT, sum(0) as DT_CTX_TDTT, sum(0) as DT_CTX_PT
                            , sum(0) as DT_CTX
                            , sum(0) as CTMTQG_DT, sum(0) as CTMTQG_TX, sum(0) as DT_CTMTCDT, sum(0) as DT_CTMTCTX,sum(0) as CTN
                            , SUM (CASE WHEN (MA_TKTN in (8991,8992,8951) and MA_MUC = ''7500'') THEN GIA_TRI_HACH_TOAN/NVL('|| DONVI_TIEN ||',1) ELSE 0 END) AS CBS
                            , sum(0) as CCN
                        from BM54_ND31 where NGAY_HIEU_LUC >= TO_DATE ('''||to_char(TUNGAY_HL,'ddMMyyyy')||''', ''ddMMyyyy'')
                        AND NGAY_HIEU_LUC <= TO_DATE ('''||to_char(DENNGAY_HL,'ddMMyyyy')||''', ''ddMMyyyy'')     
                        and NGAY_KET_SO <=TO_DATE ('''||to_char(DENNGAY_KS,'ddMMyyyy')||''', ''ddMMyyyy'')
                        and NGAY_KET_SO >=TO_DATE ('''||to_char(TUNGAY_KS,'ddMMyyyy')||''', ''ddMMyyyy'')
                        AND NOT MA_DVQHNS LIKE ''7%'' AND MA_CAPMLNS = '''||P_CAP||'''
                        ...
                        select Cast(''IV'' as nvarchar2(50)) as MA_CHUONG,Cast(''CHI DỰ PHÒNG NGÂN SÁCH'' as nvarchar2(50)) as TEN_CHUONG
                            ...
                            , sum(0) as DT_CTX_GD, sum(0) as DT_CTX_YT, sum(0) as DT_CTX_TDTT, sum(0) as DT_CTX_PT
                            , sum(0) as DT_CTX
                            , sum(0) as CTMTQG_DT, sum(0) as CTMTQG_TX, sum(0) as DT_CTMTCDT, sum(0) as DT_CTMTCTX, sum(0) as CTN, sum(0) as CBS, sum(0) as CCN
                        from dual
                        ...
                        select Cast(''VII'' as nvarchar2(50)) as MA_CHUONG,Cast(''CHI CHUYỂN NGUỒN SANG NGÂN SÁCH NĂM SAU'' as nvarchar2(50)) as TEN_CHUONG
                            ...
                            , sum(0) as DT_CTX_GD, sum(0) as DT_CTX_YT, sum(0) as DT_CTX_TDTT, sum(0) as DT_CTX_PT
                            , sum(0) as DT_CTX
                            , sum(0) as CTMTQG_DT, sum(0) as CTMTQG_TX, sum(0) as DT_CTMTCDT, sum(0) as DT_CTMTCTX, sum(0) as CTN, sum(0) as CBS
