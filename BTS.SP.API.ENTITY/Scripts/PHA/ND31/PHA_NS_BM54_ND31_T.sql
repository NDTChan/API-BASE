create or replace PROCEDURE PHA_NS_BM54_ND31 (P_MA_DBHC IN NVARCHAR2,P_LOAI_BAOCAO IN VARCHAR2, P_CONGTHUC VARCHAR2, TUNGAY_KS IN DATE, DENNGAY_KS IN DATE, TUNGAY_HL IN DATE, DENNGAY_HL IN DATE, P_CAP VARCHAR2, DONVI_TIEN number, cur OUT SYS_REFCURSOR) as
   QUERY_STR  CLOB; 
   INSERT_STR  CLOB; 
   INSERT_STR1  CLOB;
   QUERY_STR_CHI CLOB;
...
--    END IF;
     IF(P_MA_DBHC <> 27) THEN
        ...
        P_CT:=P_CT;
        END IF;
    QUERY_STR:= ' SELECT HT.MA_CHUONG, HT.TEN_CHUONG, HT.MA_DVQHNS, HT.TEN_DVQHNS, HT.MA_KBNN, HT.MA_DIABAN, HT.TEN_DIABAN
                ...
                FROM
                ( 
                 SELECT A.MA_CHUONG AS MA_CHUONG, A.TEN_CHUONG, A.MA_DVQHNS, A.TEN_DVQHNS, A.MA_KBNN, A.MA_DIABAN, A.TEN_DIABAN   
                ...
                and A.NGAY_KET_SO >=TO_DATE ('''||to_char(TUNGAY_KS,'ddMMyyyy')||''', ''ddMMyyyy'')                         
                    AND NOT A.MA_CAP like ''1'' AND NOT A.MA_CHUONG LIKE ''000'' AND NOT A.MA_DIABAN IN (''24'',''274'',''56'',''60'') AND A.MA_CAPMLNS = '''||P_CAP||''''
                    || P_CT
                || ' GROUP BY A.MA_CHUONG,A.TEN_CHUONG, A.MA_DVQHNS, A.TEN_DVQHNS, A.MA_KBNN, A.MA_DIABAN, A.TEN_DIABAN
                ) HT 
