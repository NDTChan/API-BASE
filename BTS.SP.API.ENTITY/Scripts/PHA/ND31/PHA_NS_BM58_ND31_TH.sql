create or replace PROCEDURE PHA_NS_BM58_ND31_TH (P_MA_DBHC IN NVARCHAR2,P_LOAI_BAOCAO IN VARCHAR2, P_CONGTHUC VARCHAR2, TUNGAY_KS IN DATE, DENNGAY_KS IN DATE, TUNGAY_HL IN DATE, DENNGAY_HL IN DATE, P_DBHC VARCHAR2, DONVI_TIEN number, cur OUT SYS_REFCURSOR) as
   QUERY_STR   CLOB; 
   QUERY_STR1  CLOB; 
   QUERY_STR2  CLOB;
   QUERY_STR3  CLOB;
...
--        END IF;
   ...
   IF(UPPER(P_LOAI_BAOCAO) = 'DH') THEN 
    ...
    END IF;
     EXECUTE IMMEDIATE 'DELETE PHA_THDT_HCSN_DT WHERE DL_BAOCAO = ''BM58_ND31''';
     QUERY_STR1:='INSERT INTO PHA_THDT_HCSN_DT (MA_TKTN,MA_NHOM,MA_TIEUNHOM,MA_MUC,MA_TIEUMUC,MA_CAP,MA_DVQHNS,MA_DIABAN,MA_CHUONG,TEN_CHUONG,MA_NGANHKT,MA_LOAI,MA_CTMTQG,MA_KBNN,MA_NGUON_NSNN,NGAY_KET_SO,NGAY_HIEU_LUC,ENTERED_DR,ENTERED_CR,ACCOUNTED_DR,ACCOUNTED_CR,ATTRIBUTE8,GIA_TRI_HACH_TOAN,MA_NVC,DL_BAOCAO)
            SELECT A.MA_TKTN,A.MA_NHOM,A.MA_TIEUNHOM,A.MA_MUC,A.MA_TIEUMUC,A.MA_CAP,A.MA_DVQHNS,A.MA_DIABAN,A.MA_CHUONG,A.TEN_CHUONG,A.MA_NGANHKT,A.MA_LOAI,A.MA_CTMTQG,A.MA_KBNN,A.MA_NGUON_NSNN,A.NGAY_KET_SO,A.NGAY_HIEU_LUC,A.ENTERED_DR,A.ENTERED_CR,A.ACCOUNTED_DR,A.ACCOUNTED_CR,A.ATTRIBUTE8,A.GIA_TRI_HACH_TOAN,A.MA_NVC,Cast(''BM58_ND31'' as nvarchar2(50)) as DL_BAOCAO from PHA_HACHTOAN_CHI A
            WHERE
     ...
     QUERY_STR4:='INSERT INTO PHA_THDT_HCSN_DT (MA_TKTN,MA_NHOM,MA_TIEUNHOM,MA_MUC,MA_TIEUMUC,MA_CAP,MA_DVQHNS,MA_DIABAN,MA_CHUONG,TEN_CHUONG,MA_NGANHKT,MA_LOAI,MA_CTMTQG,MA_KBNN,MA_NGUON_NSNN,NGAY_KET_SO,NGAY_HIEU_LUC,ENTERED_DR,ENTERED_CR,ACCOUNTED_DR,ACCOUNTED_CR,ATTRIBUTE8,GIA_TRI_HACH_TOAN,MA_NVC,DL_BAOCAO)
            SELECT A.MA_TKTN,A.MA_NHOM,A.MA_TIEUNHOM,A.MA_MUC,A.MA_TIEUMUC,A.MA_CAP,A.MA_DVQHNS,A.MA_DIABAN,A.MA_CHUONG,A.TEN_CHUONG,A.MA_NGANHKT,A.MA_LOAI,A.MA_CTMTQG,A.MA_KBNN,A.MA_NGUON_NSNN,A.NGAY_KET_SO,A.NGAY_HIEU_LUC,A.ENTERED_DR,A.ENTERED_CR,A.ACCOUNTED_DR,A.ACCOUNTED_CR,A.ATTRIBUTE8,A.GIA_TRI_HACH_TOAN,A.MA_NVC,Cast(''BM58_ND31'' as nvarchar2(50)) as DL_BAOCAO from PHA_DUTOAN A
            WHERE
            A.NGAY_HIEU_LUC >= TO_DATE ('''|| to_char(TUNGAY_HL,'ddMMyyyy') ||''', ''ddMMyyyy'')
            AND A.NGAY_HIEU_LUC <= TO_DATE ('''||to_char(DENNGAY_HL,'ddMMyyyy')||''', ''ddMMyyyy'')
            AND A.NGAY_KET_SO >=TO_DATE ('''||to_char(TUNGAY_KS,'ddMMyyyy')||''', ''ddMMyyyy'') 
