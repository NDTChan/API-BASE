create or replace PROCEDURE MBL_QTC_DB IS
   ...
   P_TABLENAME VARCHAR2(32767):= 'BM51_ND31_2017_TEMP';
  ...
  /*P_SQL_CREATE_TABLE := 'CREATE GLOBAL TEMPORARY TABLE  '||P_TABLENAME||'(SAPXEP  NVARCHAR2(10) , CT NVARCHAR2(2000),CAP NUMBER(10,0), INDAM NUMBER(10,0), INNGHIENG NUMBER(10,0),HIENTHI NVARCHAR2(1), TRANG_THAI NVARCHAR2(10),LOAI_CHITIEU NUMBER(10,0),MA_CHITIEU NVARCHAR2(50),MA_CHITIEU_2 NVARCHAR2(50),TEN_CHITIEU NVARCHAR2(500),' 
    ...
    FOR n IN thang.FIRST .. thang.LAST
       LOOP
            ...
            P_DELETE:= 'DELETE FROM MBL_PHA_QTTDB WHERE NAM ='||CURREN_YEAR||' AND SHKB = '||dbhc(m)||' AND THANG = '||thangNum(n)||' AND LOAIQT = ''C''';
            P_INSERT:='INSERT INTO MBL_PHA_QTTDB (NAM,SHKB,THANG,LOAIQT) VALUES('||CURREN_YEAR||','||dbhc(m)||','||thangNum(n)||',''C'')';
            BEGIN
  ...
  --- B2 UPDATE CONG THUC TONG TAI CAC CHI TIEU CO TRANG THAI S 2
    ...
    -- update row co trang thai =CTMTQG=SUM (CTMTQT_C_)        
      ...
      P_UPDATE :='UPDATE MBL_PHA_QTTDB SET
                        DT = (SELECT DT FROM ' ||P_TABLENAME||' WHERE MA_CHITIEU = ''100000_C''),
                        QT = (SELECT QT FROM ' ||P_TABLENAME||' WHERE MA_CHITIEU = ''100000_C'')
                        WHERE NAM ='||CURREN_YEAR||' AND SHKB = '||dbhc(m)||' AND THANG = '||thangNum(n)||' AND LOAIQT =''C''';
