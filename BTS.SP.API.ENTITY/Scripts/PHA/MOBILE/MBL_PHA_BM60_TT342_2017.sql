create or replace PROCEDURE MBL_PHA_BM60_TT342_2017 IS
   ...
   P_TABLENAME VARCHAR2(100):='BM60_TT342_2017';
   MABAOCAO  VARCHAR2(100):='BM60_TT342_2017';
  ...
  P_UPDATE VARCHAR2(32767);
   ...
   LOOP
        ...
        LOOP
                    ...
                    V_TU_NAM:= nam(n);
                    P_DELETE:= 'DELETE FROM MBL_PHA_C_NSDP WHERE NAM ='||nam(n)||' AND SHKB = '||dbhc(m)||' AND THANG = '||thangNum(n)||'';
                    P_INSERT:='INSERT INTO MBL_PHA_C_NSDP (NAM,SHKB,THANG) VALUES('||nam(n)||','||dbhc(m)||','||thangNum(n)||')';
                    BEGIN
    ...
    LOOP      
        ...
        BEGIN
                ...
                FOR item_C IN (SELECT MA_COT,CONGTHUC_WHERE,TRANG_THAI FROM DM_CHITIEU_BAOCAO_COL WHERE MA_BAOCAO=MABAOCAO ORDER BY STT ASC)
                    ...
                    -- if(item_R.MA_CHITIEU = '30130')then
...
-- B2 UPDATE CONG THUC TONG TAI CAC CHI TIEU CO TRANG THAI S 1
  ...
  --- B2 UPDATE CONG THUC TONG TAI CAC CHI TIEU CO TRANG THAI S 2
    ...
    -- update row co trang thai =CTMTQG=SUM (CTMTQT_C_)        
       ...
       ----DBMS_OUTPUT.ENABLE (buffer_size => NULL);
     ----DBMS_OUTPUT.put_line ('t:=' ||  P_UPDATE_SUM );

       EXECUTE IMMEDIATE P_UPDATE_SUM; 
        P_UPDATE:='';
      P_UPDATE :='UPDATE MBL_PHA_C_NSDP SET
                        T_CCD = (SELECT NST FROM ' ||P_TABLENAME||' WHERE MA_CHITIEU = ''10000_C''),
                        T_CTN = (SELECT NST FROM ' ||P_TABLENAME||' WHERE MA_CHITIEU = ''49000''),
                        H_CCD = (SELECT NSH FROM ' ||P_TABLENAME||' WHERE MA_CHITIEU = ''10000_C''),
                        H_CTN = (SELECT NSH FROM ' ||P_TABLENAME||' WHERE MA_CHITIEU = ''49000''),
                        X_CCD = (SELECT NSX FROM ' ||P_TABLENAME||' WHERE MA_CHITIEU = ''10000_C''),
                        X_CTN = (SELECT NSX FROM ' ||P_TABLENAME||' WHERE MA_CHITIEU = ''49000'')
