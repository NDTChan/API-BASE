...
--------------------------------------------------------
--------------------------------------------------------
--  DDL for Procedure MBL_PHA_MAUBS_02_TEST
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "BTSTC"."MBL_PHA_MAUBS_02_TEST" as
   ...
   P_MA_DBHC NVARCHAR2(10) :='27';
   DONVI_TIEN number:=1;
   P_LOAI_BAOCAO VARCHAR2(10) := 'DH';
   P_LOAI_DL VARCHAR2(10):= 'CANDOI';
       CURRENT_DATET  DATE := SYSDATE;
        CURRENT_YEAR NUMBER := TO_NUMBER(to_char(CURRENT_DATET,'yyyy'));
    TUNGAY_KS  DATE:= TO_DATE('01-JAN-'||CURRENT_YEAR,'dd-MM-yyyy');
    DENNGAY_KS  DATE:=CURRENT_DATET;
    TUNGAY_HL  DATE:=TO_DATE('01-JAN-'||CURRENT_YEAR,'dd-MM-yyyy');
    DENNGAY_HL  DATE:=CURRENT_DATET;
 ...
 TEMP_THU_DB decimal(18,2);
   ...
   P_WHERE_DATE_KS_TU VARCHAR2(100);
   ...
   BEGIN
    ...
    END IF;
    BEGIN
        DELETE FROM  MBL_PHA_BIEU1 Where MA_DBHC = ''||P_MA_DBHC||'' AND TU_NGAYHL = ''||TUNGAY_HL||'';
        Dbms_Output.Put_Line('DELETE FROM  MBL_PHA_BIEU1 Where MA_DBHC = '''||P_MA_DBHC||''' AND TU_NGAYHL = '''||TUNGAY_HL||''';');
        --EXECUTE IMMEDIATE abcabc;  
        INSERT INTO MBL_PHA_BIEU1 (MA_DBHC,TU_NGAYHL) VALUES (''||P_MA_DBHC||'',''||TUNGAY_HL||'');  
    END;
   ...
   -- COT1 - BS-02 --
    ...
    END IF;
            ...
            QUERY_STR5:='
            UPDATE MBL_PHA_BIEU1 SET THU_DB =
            ...
            )K) WHERE MA_DBHC = '''||P_MA_DBHC||''' AND TU_NGAYHL = '''||TUNGAY_HL||''' ';
               -- END COT1 - BS-02 --
