create or replace PROCEDURE PHA_B10_DTXDCB_CX (P_MA_DBHC IN NVARCHAR2,P_CONGTHUC VARCHAR2, TUNGAY_KS IN DATE, DENNGAY_KS IN DATE, TUNGAY_HL IN DATE, DENNGAY_HL IN DATE,DONVI_TIEN number, cur OUT SYS_REFCURSOR) as
...
QUERY_STR:=' SELECT LT.MA,LT.MA_DVQHNS,LT.TEN_DVQHNS,LT.DUTOAN, LT.CHI, LT.UNG, LT.TEN, LT.MA_DVQHNS_CHA , DV2.TEN_DVQHNS
            ...
            (SELECT KB.MA,CT.MA_DVQHNS,CT.TEN_DVQHNS,CT.DUTOAN, CT.CHI, CT.UNG, KB.TEN AS TEN FROM 
                (SELECT HT.MA_DVQHNS, HT.TEN_DVQHNS,  HT.MA_KBNN, HT.MA_CHUONG
                ...
                and B.NGAY_KET_SO >=TO_DATE ('''||to_char(TUNGAY_KS,'ddMMyyyy')||''', ''ddMMyyyy'') AND NOT B.MA_DIABAN IN (''27'',''256'',''258'',''259'',''260'',''261'',''262'',''263'',''264'') AND B.MA_CAP LIKE ''4'')     THEN B.GIA_TRI_HACH_TOAN/NVL('|| DONVI_TIEN ||',1) ELSE 0 END),0) AS CHI
               ,NVL(SUM (0),0) AS UNG
                ...
                WHERE 1=1 AND HT.DUTOAN <> 0
                 GROUP BY HT.MA_DVQHNS, HT.TEN_DVQHNS, HT.DUTOAN, HT.MA_KBNN, HT.MA_CHUONG) CT 
                ...
                AND B.NGAY_HIEU_LUC <= TO_DATE ('''||to_char(DENNGAY_HL,'ddMMyyyy')||''', ''ddMMyyyy'')     
                and B.NGAY_KET_SO <=TO_DATE ('''||to_char(DENNGAY_KS,'ddMMyyyy')||''', ''ddMMyyyy'')
                and B.NGAY_KET_SO >=TO_DATE ('''||to_char(TUNGAY_KS,'ddMMyyyy')||''', ''ddMMyyyy'') AND NOT B.MA_DIABAN IN (''27'',''256'',''258'',''259'',''260'',''261'',''262'',''263'',''264'') AND B.MA_CAP LIKE ''4'')     THEN B.GIA_TRI_HACH_TOAN/NVL('|| DONVI_TIEN ||',1) ELSE 0 END),0) AS UNG
                FROM
               PHA_HACHTOAN_CHI B 
               WHERE 1=1 GROUP BY B.MA_CHUONG, B.MA_DVQHNS, B.TEN_DVQHNS,  B.MA_KBNN) CT
                ...
                ,NVL(HT.CHI,0) as CHI
                ,NVL(SUM (CASE WHEN (B.MA_TKTN LIKE ''9%'' AND NOT B.MA_TKTN IN (''9557'',''9567'')  AND B.NGAY_HIEU_LUC >= TO_DATE ('''||to_char(TUNGAY_HL,'ddMMyyyy')||''', ''ddMMyyyy'')
                AND B.NGAY_HIEU_LUC <= TO_DATE ('''||to_char(DENNGAY_HL,'ddMMyyyy')||''', ''ddMMyyyy'')     
                and B.NGAY_KET_SO <=TO_DATE ('''||to_char(DENNGAY_KS,'ddMMyyyy')||''', ''ddMMyyyy'')
                and B.NGAY_KET_SO >=TO_DATE ('''||to_char(TUNGAY_KS,'ddMMyyyy')||''', ''ddMMyyyy'')  AND B.MA_CAP LIKE ''4'')     THEN B.GIA_TRI_HACH_TOAN/NVL('|| DONVI_TIEN ||',1) ELSE 0 END),0) AS DUTOAN
               ,NVL(SUM (0),0) AS UNG
                ...
                AND A.MA_DVQHNS LIKE ''7%''
                AND A.MA_CAP LIKE ''4''
                AND NOT A.MA_DIABAN IN (''27'',''256'',''258'',''259'',''260'',''261'',''262'',''263'',''264'')
                '|| P_SQL_INSERT|| ' GROUP BY A.MA_CHUONG, A.MA_DVQHNS, A.TEN_DVQHNS, A.MA_KBNN
                ) HT LEFT JOIN PHA_DUTOAN B ON HT.MA_DVQHNS = B.MA_DVQHNS AND HT.MA_CHUONG = B.MA_CHUONG AND HT.MA_KBNN = B.MA_KBNN
                WHERE 1=1
                 GROUP BY HT.MA_DVQHNS, HT.TEN_DVQHNS, HT.CHI, HT.MA_KBNN) CT 
                 INNER JOIN DM_TKKHOBAC KB ON CT.MA_KBNN = KB.MA AND (CT.CHI <> 0 AND CT.DUTOAN = 0)
                GROUP BY KB.MA,CT.MA_DVQHNS,CT.TEN_DVQHNS,CT.DUTOAN, CT.CHI, CT.UNG, KB.TEN) LAST INNER JOIN SYS_DVQHNS DV ON LAST.MA_DVQHNS = DV.MA_DVQHNS
                GROUP BY LAST.MA,LAST.MA_DVQHNS,LAST.TEN_DVQHNS,LAST.DUTOAN, LAST.CHI, LAST.UNG, LAST.TEN, DV.MA_DVQHNS_CHA) LT INNER JOIN SYS_DVQHNS DV2 ON LT.MA_DVQHNS_CHA = DV2.MA_DVQHNS
                GROUP BY LT.MA,LT.MA_DVQHNS,LT.TEN_DVQHNS,LT.DUTOAN, LT.CHI, LT.UNG, LT.TEN, LT.MA_DVQHNS_CHA , DV2.TEN_DVQHNS
                ORDER BY TEN 
