create or replace PROCEDURE PHA_BM66_TT343 (P_MA_DBHC IN NVARCHAR2,P_LOAI_BAOCAO IN VARCHAR2, P_CONGTHUC VARCHAR2, TUNGAY_KS IN DATE, DENNGAY_KS IN DATE, TUNGAY_HL IN DATE, DENNGAY_HL IN DATE, P_CAP VARCHAR2, DONVI_TIEN number, cur OUT SYS_REFCURSOR) as
   QUERY_STR  CLOB;
   QUERY_STR1  CLOB; 
   QUERY_STR2  CLOB;
   INSERT_STR  CLOB; 
   ...
   --KHAC
   CT_CTMTCDT VARCHAR2(32767);   CT_CTMTCTX VARCHAR2(32767);   CT_CTNL VARCHAR2(32767);   CT_CBS VARCHAR2(32767); CT_CCN VARCHAR2(32767);
   ...
   BEGIN
    ...
    END IF;
    -------KHAC
     select REPLACE(CONGTHUC_DH_WHERE, '''', '''') INTO CT_CTNL from DM_CHITIEU_BAOCAO_COL WHERE MA_BAOCAO='BM66_TT343' AND MA_COT='40000';
     select REPLACE(CONGTHUC_DH_WHERE, '''', '''') INTO CT_CBS from DM_CHITIEU_BAOCAO_COL WHERE MA_BAOCAO='BM66_TT343' AND MA_COT='41000';
     select REPLACE(CONGTHUC_DH_WHERE, '''', '''') INTO CT_CCN from DM_CHITIEU_BAOCAO_COL WHERE MA_BAOCAO='BM66_TT343' AND MA_COT='48000';
     ...
     P_SQL_CREATE_TABLE_TH := 'CREATE TABLE BM66_TT343(
  ...
  ';          
    ...
    QUERY_STR_CHI := 'SELECT 2,MA_TKTN ,MA_DIABAN, TEN_DIABAN,MA_CAP,MA_CAPMLNS,MA_CHUONG,TEN_CHUONG,MA_NGANHKT,MA_LOAI,MA_TIEUMUC,MA_MUC,MA_TIEUNHOM,MA_NHOM,MA_CTMTQG,MA_KBNN,MA_NGUON_NSNN,MA_DVQHNS, TEN_DVQHNS,MA_NVC,TO_DATE(to_char(NGAY_HIEU_LUC,''ddMMyyyy''), ''ddMMyyyy''),TO_DATE (to_char(NGAY_KET_SO,''ddMMyyyy''), ''ddMMyyyy''),ATTRIBUTE8
        ...
        INSERT_STR := 'INSERT INTO BM66_TT343(LOAI_CHITIEU,MA_TKTN ,MA_DIABAN, TEN_DIABAN,MA_CAP,MA_CAPMLNS,MA_CHUONG,TEN_CHUONG,MA_NGANHKT,MA_LOAI, MA_TIEUMUC,MA_MUC,MA_TIEUNHOM,MA_NHOM,MA_CTMTQG,MA_KBNN,MA_NGUON_NSNN,MA_DVQHNS, TEN_DVQHNS,MA_NVC,NGAY_HIEU_LUC,NGAY_KET_SO,LOAI_DU_TOAN,GIA_TRI_HACH_TOAN)('||QUERY_STR_CHI||')';
   ...
   QUERY_STR_DUTOAN := 'SELECT 3,MA_TKTN ,MA_DIABAN, TEN_DIABAN,MA_CAP,MA_CAPMLNS,MA_CHUONG,TEN_CHUONG,MA_NGANHKT,MA_LOAI,MA_TIEUMUC,MA_MUC,MA_TIEUNHOM,MA_NHOM,MA_CTMTQG,MA_KBNN,MA_NGUON_NSNN,MA_DVQHNS, TEN_DVQHNS,MA_NVC,TO_DATE(to_char(NGAY_HIEU_LUC,''ddMMyyyy''), ''ddMMyyyy''),TO_DATE (to_char(NGAY_KET_SO,''ddMMyyyy''), ''ddMMyyyy''),ATTRIBUTE8
        ,SUM (GIA_TRI_HACH_TOAN) FROM PHA_DUTOAN VC 
         GROUP BY  MA_TKTN,MA_DIABAN, TEN_DIABAN,MA_CAP,MA_CAPMLNS,MA_CHUONG,TEN_CHUONG,MA_NGANHKT,MA_LOAI,MA_TIEUMUC,MA_MUC,MA_TIEUNHOM,MA_NHOM,MA_CTMTQG,MA_KBNN,MA_NGUON_NSNN,MA_DVQHNS, TEN_DVQHNS,MA_NVC,NGAY_HIEU_LUC,NGAY_KET_SO,ATTRIBUTE8';   
        INSERT_STR1 := 'INSERT INTO BM66_TT343(LOAI_CHITIEU,MA_TKTN ,MA_DIABAN, TEN_DIABAN,MA_CAP,MA_CAPMLNS,MA_CHUONG,TEN_CHUONG,MA_NGANHKT,MA_LOAI, MA_TIEUMUC,MA_MUC,MA_TIEUNHOM,MA_NHOM,MA_CTMTQG,MA_KBNN,MA_NGUON_NSNN,MA_DVQHNS, TEN_DVQHNS,MA_NVC,NGAY_HIEU_LUC,NGAY_KET_SO,LOAI_DU_TOAN,GIA_TRI_HACH_TOAN)('||QUERY_STR_DUTOAN||')';
  BEGIN
        SELECT COUNT(*)  INTO N_COUNT  FROM ALL_TAB_COLUMNS  WHERE TABLE_NAME = 'BM66_TT343';
    ...
    END IF;
        ...
        END IF;
            ...
            select Cast(''1'' as nvarchar2(50)) as MA_CHUONG,Cast(''CÁC CƠ QUAN, TỔ CHỨC'' as nvarchar2(50)) as TEN_CHUONG
                        , sum(0) as CT_CDT_QP, sum(0) as CT_CDT_AN, sum(0) as CT_CDT_GDDT, sum(0) as CT_CDT_KHCN, sum(0) as CT_CDT_YT
                        , sum(0) as CT_CDT_VHTT, sum(0) as CT_CDT_PTTH, sum(0) as CT_CDT_TDTT, sum(0) as CT_CDT_BVMT
                        , sum(0) as CT_CDT_KT, sum(0) as CT_CDT_QLNN, sum(0) as CT_CDT_BDXH, sum(0) as CT_CDT_KHAC
                        , sum(0) as CT_DT_CDT_QP, sum(0) as CT_DT_CDT_AN, sum(0) as CT_DT_CDT_GDDT, sum(0) as CT_DT_CDT_KHCN, sum(0) as CT_DT_CDT_YT
                        , sum(0) as CT_DT_CDT_VHTT, sum(0) as CT_DT_CDT_PTTH, sum(0) as CT_DT_CDT_TDTT, sum(0) as CT_DT_CDT_BVMT
                        , sum(0) as CT_DT_CDT_KT, sum(0) as CT_DT_CDT_QLNN, sum(0) as CT_DT_CDT_BDXH, sum(0) as CT_DT_CDT_KHAC
                        , sum(0) as CT_CTX_QP, sum(0) as  CT_CTX_AN, sum(0) as  CT_CTX_GDDT
                        , sum(0) as CT_CTX_KHCN
                        , sum(0) as CT_CTX_YT, sum(0) as CT_CTX_VHTT
                        , sum(0) as CT_CTX_PTTH, sum(0) as CT_CTX_TDTT, sum(0) as CT_CTX_BVMT, sum(0) as CT_CTX_KT
                        , sum(0) as CT_CTX_QLNN, sum(0) as CT_CTX_BDXH, sum(0) as CT_CTX_KHAC
                        , sum(0) as CT_DT_CTX_QP, sum(0) as  CT_DT_CTX_AN, sum(0) as CT_DT_CTX_GDDT, sum(0) as CT_DT_CTX_KHCN, sum(0) as CT_DT_CTX_YT
                        , sum(0) as CT_DT_CTX_VHTT, sum(0) as CT_DT_CTX_PTTH, sum(0) as CT_DT_CTX_TDTT, sum(0) as CT_DT_CTX_BVMT
                        , sum(0) as CT_DT_CTX_KT, sum(0) as CT_DT_CTX_QLNN, sum(0) as CT_DT_CTX_BDXH, sum(0) as CT_DT_CTX_KHAC
                        , sum(0) as CT_CTMTCDT
                        , sum(0) as CT_CTMTCTX, sum(0) as CT_DT_CTMTCDT
                        , sum(0) as CT_DT_CTMTCTX, sum(0) as CT_CTNL
                        , sum(0) as CT_CBS, sum(0) as CT_CCN
            ...
            SELECT HT.MA_CHUONG, HT.TEN_CHUONG
                        ...
                        , NVL(HT.CT_DT_CTMTCTX,0) as CT_DT_CTMTCTX, NVL(HT.CT_CTNL,0) as CT_CTNL
                        , NVL(HT.CT_CBS,0) as CT_CBS, NVL(HT.CT_CCN,0) as CT_CCN 
                        FROM
                        (
                        ...
                        select * from 
                        (                       
                        select Cast(''II'' as nvarchar2(50)) as MA_CHUONG, Cast(''CHI TRẢ NỢ LÃI CÁC KHOẢN DO CHÍNH QUYỀN ĐỊA PHƯƠNG VAY'' as nvarchar2(50)) as TEN_CHUONG
                        ,  sum(0) as CT_CDT_QP,  sum(0) as CT_CDT_AN,  sum(0) as CT_CDT_GDDT,  sum(0) as CT_CDT_KHCN,  sum(0) as CT_CDT_YT
                        ,  sum(0) as CT_CDT_VHTT,  sum(0) as CT_CDT_PTTH,  sum(0) as CT_CDT_TDTT,  sum(0) as CT_CDT_BVMT
                        ,  sum(0) as CT_CDT_KT,  sum(0) as CT_CDT_QLNN,  sum(0) as CT_CDT_BDXH,  sum(0) as CT_CDT_KHAC
                        ,  sum(0) as CT_DT_CDT_QP,  sum(0) as CT_DT_CDT_AN,  sum(0) as CT_DT_CDT_GDDT,  sum(0) as CT_DT_CDT_KHCN,  sum(0) as CT_DT_CDT_YT
                        ,  sum(0) as CT_DT_CDT_VHTT,  sum(0) as CT_DT_CDT_PTTH,  sum(0) as CT_DT_CDT_TDTT, sum(0) as CT_DT_CDT_BVMT
                        ,  sum(0) as CT_DT_CDT_KT,  sum(0) as CT_DT_CDT_QLNN,  sum(0) as CT_DT_CDT_BDXH,  sum(0) as CT_DT_CDT_KHAC 
                        ,  sum(0) as CT_CTX_QP,  sum(0) as  CT_CTX_AN,  sum(0) as  CT_CTX_GDDT,  sum(0) as  CT_CTX_KHCN, sum(0) as CT_CTX_YT,  sum(0) as CT_CTX_VHTT
                        ,  sum(0) as CT_CTX_PTTH,  sum(0) as CT_CTX_TDTT,  sum(0) as CT_CTX_BVMT,  sum(0) as CT_CTX_KT
                        ,  sum(0) as CT_CTX_QLNN,  sum(0) as CT_CTX_BDXH,  sum(0) as CT_CTX_KHAC
                        ,  sum(0) as  CT_DT_CTX_QP,  sum(0) as  CT_DT_CTX_AN,  sum(0) as CT_DT_CTX_GDDT,  sum(0) as CT_DT_CTX_KHCN,  sum(0) as CT_DT_CTX_YT
                        ,  sum(0) as CT_DT_CTX_VHTT,  sum(0) as CT_DT_CTX_PTTH,  sum(0) as CT_DT_CTX_TDTT,  sum(0) as CT_DT_CTX_BVMT
                        ...
                        ,  sum(0) AS CT_DT_CTMTCDT
                        ,  sum(0) AS CT_DT_CTMTCTX
                        ,  NVL(SUM (CASE WHEN ('|| CT_CTNL ||')        THEN GIA_TRI_HACH_TOAN/NVL('|| DONVI_TIEN ||',1) ELSE 0 END),0) AS CT_CTNL
                        ,  sum(0) as CT_CBS,  sum(0) as CT_CCN
                        from BM66_TT343 where MA_CAP <> 1 AND NGAY_HIEU_LUC >= TO_DATE ('''||to_char(TUNGAY_HL,'ddMMyyyy')||''', ''ddMMyyyy'')
                        ...
                        AND NOT MA_DVQHNS LIKE ''7%''

                        union all
                        select Cast(''III'' as nvarchar2(50)) as MA_CHUONG, Cast(''CHI BỔ SUNG QUỸ DỰ TRỮ TÀI CHÍNH'' as nvarchar2(50)) as TEN_CHUONG
                        ,  sum(0) as CT_CDT_QP,  sum(0) as CT_CDT_AN,  sum(0) as CT_CDT_GDDT,  sum(0) as CT_CDT_KHCN,  sum(0) as CT_CDT_YT
                        ,  sum(0) as CT_CDT_VHTT,  sum(0) as CT_CDT_PTTH,  sum(0) as CT_CDT_TDTT,  sum(0) as CT_CDT_BVMT
                        ,  sum(0) as CT_CDT_KT,  sum(0) as CT_CDT_QLNN,  sum(0) as CT_CDT_BDXH,  sum(0) as CT_CDT_KHAC
                        ,  sum(0) as CT_DT_CDT_QP,  sum(0) as CT_DT_CDT_AN,  sum(0) as CT_DT_CDT_GDDT,  sum(0) as CT_DT_CDT_KHCN,  sum(0) as CT_DT_CDT_YT
                        ...
                        ,  sum(0) AS CT_DT_CTMTCTX
                        ,  sum(0) as CT_CTNL
                        ,  NVL(SUM (CASE WHEN ('|| CT_CBS ||')        THEN GIA_TRI_HACH_TOAN/NVL('|| DONVI_TIEN ||',1) ELSE 0 END),0) AS CT_CBS
                        ,  sum(0) as CT_CCN
                        from BM66_TT343 where NGAY_HIEU_LUC >= TO_DATE ('''||to_char(TUNGAY_HL,'ddMMyyyy')||''', ''ddMMyyyy'')
                        AND NGAY_HIEU_LUC <= TO_DATE ('''||to_char(DENNGAY_HL,'ddMMyyyy')||''', ''ddMMyyyy'')     
                        and NGAY_KET_SO <=TO_DATE ('''||to_char(DENNGAY_KS,'ddMMyyyy')||''', ''ddMMyyyy'')
                        and NGAY_KET_SO >=TO_DATE ('''||to_char(TUNGAY_KS,'ddMMyyyy')||''', ''ddMMyyyy'')
                        AND NOT MA_DVQHNS LIKE ''7%''
                        ...
                        union all
                        select Cast(''V'' as nvarchar2(50)) as MA_CHUONG,Cast(''CHI TẠO NGUỒN, ĐIỀU CHỈNH TIỀN LƯƠNG'' as nvarchar2(50)) as TEN_CHUONG
                        ,  sum(0) as CT_CDT_QP,  sum(0) as CT_CDT_AN,  sum(0) as CT_CDT_GDDT,  sum(0) as CT_CDT_KHCN,  sum(0) as CT_CDT_YT
                        ,  sum(0) as CT_CDT_VHTT,  sum(0) as CT_CDT_PTTH,  sum(0) as CT_CDT_TDTT,  sum(0) as CT_CDT_BVMT
                        ,  sum(0) as CT_CDT_KT,  sum(0) as CT_CDT_QLNN,  sum(0) as CT_CDT_BDXH,  sum(0) as CT_CDT_KHAC
                        ,  sum(0) as CT_DT_CDT_QP,  sum(0) as CT_DT_CDT_AN,  sum(0) as CT_DT_CDT_GDDT,  sum(0) as CT_DT_CDT_KHCN,  sum(0) as CT_DT_CDT_YT
                        ...
                        union all                        
                        select Cast(''VI'' as nvarchar2(50)) as MA_CHUONG,Cast(''CHI BỔ SUNG CÓ MỤC TIÊU CHO NGÂN SÁCH HUYỆN '' as nvarchar2(50)) as TEN_CHUONG
                        ,  sum(0) as CT_CDT_QP,  sum(0) as CT_CDT_AN,  sum(0) as CT_CDT_GDDT,  sum(0) as CT_CDT_KHCN,  sum(0) as CT_CDT_YT
                        ,  sum(0) as CT_CDT_VHTT,  sum(0) as CT_CDT_PTTH,  sum(0) as CT_CDT_TDTT,  sum(0) as CT_CDT_BVMT
                        ,  sum(0) as CT_CDT_KT,  sum(0) as CT_CDT_QLNN,  sum(0) as CT_CDT_BDXH,  sum(0) as CT_CDT_KHAC
                        ,  sum(0) as CT_DT_CDT_QP,  sum(0) as CT_DT_CDT_AN,  sum(0) as CT_DT_CDT_GDDT,  sum(0) as CT_DT_CDT_KHCN,  sum(0) as CT_DT_CDT_YT
                        ...
                        ,  sum(0) as CT_CTNL
                        ,  sum(0) as CT_CBS
                        ,  NVL(SUM (CASE WHEN ('|| CT_CCN ||')        THEN GIA_TRI_HACH_TOAN/NVL('|| DONVI_TIEN ||',1) ELSE 0 END),0) AS CT_CCN  
                        from BM66_TT343 where NGAY_HIEU_LUC >= TO_DATE ('''||to_char(TUNGAY_HL,'ddMMyyyy')||''', ''ddMMyyyy'')
                        AND NGAY_HIEU_LUC <= TO_DATE ('''||to_char(DENNGAY_HL,'ddMMyyyy')||''', ''ddMMyyyy'')     
                        and NGAY_KET_SO <=TO_DATE ('''||to_char(DENNGAY_KS,'ddMMyyyy')||''', ''ddMMyyyy'')
                        and NGAY_KET_SO >=TO_DATE ('''||to_char(TUNGAY_KS,'ddMMyyyy')||''', ''ddMMyyyy'')
