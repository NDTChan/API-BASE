create or replace PROCEDURE PHA_BM86_TT343 (P_MA_DBHC IN NVARCHAR2,P_LOAI_BAOCAO IN VARCHAR2, P_CONGTHUC VARCHAR2, TUNGAY_KS IN DATE, DENNGAY_KS IN DATE, TUNGAY_HL IN DATE, DENNGAY_HL IN DATE, P_CAP VARCHAR2, DONVI_TIEN number, cur OUT SYS_REFCURSOR) as
   QUERY_STR  CLOB;
   QUERY_STR1  CLOB; 
   QUERY_STR2  CLOB;
   INSERT_STR  CLOB; 
   ...
   --KHAC
   CT_CTMTCDT VARCHAR2(32767);   CT_CTMTCTX VARCHAR2(32767);   CDP VARCHAR2(32767);   CTN VARCHAR2(32767); CT_CCN VARCHAR2(32767);
   ...
   BEGIN
    ...
    END IF;
    -------KHAC
     select REPLACE(CONGTHUC_DH_WHERE, '''', '''') INTO CDP from DM_CHITIEU_BAOCAO_COL WHERE MA_BAOCAO='BM86_TT343' AND MA_COT='43000';
     select REPLACE(CONGTHUC_DH_WHERE, '''', '''') INTO CTN from DM_CHITIEU_BAOCAO_COL WHERE MA_BAOCAO='BM86_TT343' AND MA_COT='44000';
     ...
     P_SQL_CREATE_TABLE_TH := 'CREATE TABLE BM86_TT343(
  ...
  ';          
    ...
    QUERY_STR_CHI := 'SELECT 2,MA_TKTN ,MA_DIABAN, TEN_DIABAN,MA_CAP,MA_CAPMLNS,MA_CHUONG,TEN_CHUONG,MA_NGANHKT,MA_LOAI,MA_TIEUMUC,MA_MUC,MA_TIEUNHOM,MA_NHOM,MA_CTMTQG,MA_KBNN,MA_NGUON_NSNN,MA_DVQHNS, TEN_DVQHNS,MA_NVC,TO_DATE(to_char(NGAY_HIEU_LUC,''ddMMyyyy''), ''ddMMyyyy''),TO_DATE (to_char(NGAY_KET_SO,''ddMMyyyy''), ''ddMMyyyy''),ATTRIBUTE8
        ...
        INSERT_STR := 'INSERT INTO BM86_TT343(LOAI_CHITIEU,MA_TKTN ,MA_DIABAN, TEN_DIABAN,MA_CAP,MA_CAPMLNS,MA_CHUONG,TEN_CHUONG,MA_NGANHKT,MA_LOAI, MA_TIEUMUC,MA_MUC,MA_TIEUNHOM,MA_NHOM,MA_CTMTQG,MA_KBNN,MA_NGUON_NSNN,MA_DVQHNS, TEN_DVQHNS,MA_NVC,NGAY_HIEU_LUC,NGAY_KET_SO,LOAI_DU_TOAN,GIA_TRI_HACH_TOAN)('||QUERY_STR_CHI||')';
   ...
   QUERY_STR_DUTOAN := 'SELECT 3,MA_TKTN ,MA_DIABAN, TEN_DIABAN,MA_CAP,MA_CAPMLNS,MA_CHUONG,TEN_CHUONG,MA_NGANHKT,MA_LOAI,MA_TIEUMUC,MA_MUC,MA_TIEUNHOM,MA_NHOM,MA_CTMTQG,MA_KBNN,MA_NGUON_NSNN,MA_DVQHNS, TEN_DVQHNS,MA_NVC,TO_DATE(to_char(NGAY_HIEU_LUC,''ddMMyyyy''), ''ddMMyyyy''),TO_DATE (to_char(NGAY_KET_SO,''ddMMyyyy''), ''ddMMyyyy''),ATTRIBUTE8
        ,SUM (GIA_TRI_HACH_TOAN) FROM PHA_DUTOAN VC 
         GROUP BY  MA_TKTN,MA_DIABAN, TEN_DIABAN,MA_CAP,MA_CAPMLNS,MA_CHUONG,TEN_CHUONG,MA_NGANHKT,MA_LOAI,MA_TIEUMUC,MA_MUC,MA_TIEUNHOM,MA_NHOM,MA_CTMTQG,MA_KBNN,MA_NGUON_NSNN,MA_DVQHNS, TEN_DVQHNS,MA_NVC,NGAY_HIEU_LUC,NGAY_KET_SO,ATTRIBUTE8';   
        INSERT_STR1 := 'INSERT INTO BM86_TT343(LOAI_CHITIEU,MA_TKTN ,MA_DIABAN, TEN_DIABAN,MA_CAP,MA_CAPMLNS,MA_CHUONG,TEN_CHUONG,MA_NGANHKT,MA_LOAI, MA_TIEUMUC,MA_MUC,MA_TIEUNHOM,MA_NHOM,MA_CTMTQG,MA_KBNN,MA_NGUON_NSNN,MA_DVQHNS, TEN_DVQHNS,MA_NVC,NGAY_HIEU_LUC,NGAY_KET_SO,LOAI_DU_TOAN,GIA_TRI_HACH_TOAN)('||QUERY_STR_DUTOAN||')';
  BEGIN
        SELECT COUNT(*)  INTO N_COUNT  FROM ALL_TAB_COLUMNS  WHERE TABLE_NAME = 'BM86_TT343';
    ...
    END IF;
        ...
        END IF;
            ...
            select Cast(''1'' as nvarchar2(50)) as MA_CHUONG,Cast(''CÁC CƠ QUAN, TỔ CHỨC'' as nvarchar2(50)) as TEN_CHUONG
                        ...
                        , sum(0) as CT_DT_CTMTCTX, sum(0) as CDP
                        , sum(0) as CTN, sum(0) as CT_CCN
            ...
            SELECT HT.MA_CHUONG, HT.TEN_CHUONG
                        ...
                        , NVL(HT.CT_DT_CTMTCTX,0) as CT_DT_CTMTCTX, NVL(HT.CDP,0) as CDP
                        , NVL(HT.CTN,0) as CTN, NVL(HT.CT_CCN,0) as CT_CCN 
                        FROM
                        (
                        ...
                        select * from 
                        (                       
                        select Cast(''II'' as nvarchar2(50)) as MA_CHUONG, Cast(''CHI DỰ PHÒNG NGÂN SÁCH'' as nvarchar2(50)) as TEN_CHUONG
                        ,  sum(0) as CT_CDT_QP,  sum(0) as CT_CDT_AN,  sum(0) as CT_CDT_GDDT,  sum(0) as CT_CDT_KHCN,  sum(0) as CT_CDT_YT
                        ,  sum(0) as CT_CDT_VHTT,  sum(0) as CT_CDT_PTTH,  sum(0) as CT_CDT_TDTT,  sum(0) as CT_CDT_BVMT
                        ,  sum(0) as CT_CDT_KT,  sum(0) as CT_CDT_QLNN,  sum(0) as CT_CDT_BDXH,  sum(0) as CT_CDT_KHAC
                        ,  sum(0) as CT_DT_CDT_QP,  sum(0) as CT_DT_CDT_AN,  sum(0) as CT_DT_CDT_GDDT,  sum(0) as CT_DT_CDT_KHCN,  sum(0) as CT_DT_CDT_YT
                        ...
                        ,  sum(0) AS CDP
                        ,  sum(0) as CTN,  sum(0) as CT_CCN
                        from BM86_TT343 where MA_CAP <> 1 AND NGAY_HIEU_LUC >= TO_DATE ('''||to_char(TUNGAY_HL,'ddMMyyyy')||''', ''ddMMyyyy'')
                        ...
                        union all
                        select Cast(''III'' as nvarchar2(50)) as MA_CHUONG, Cast(''CHI TẠO NGUỒN, ĐIỀU CHỈNH TIỀN LƯƠNG'' as nvarchar2(50)) as TEN_CHUONG
                        ,  sum(0) as CT_CDT_QP,  sum(0) as CT_CDT_AN,  sum(0) as CT_CDT_GDDT,  sum(0) as CT_CDT_KHCN,  sum(0) as CT_CDT_YT
                        ,  sum(0) as CT_CDT_VHTT,  sum(0) as CT_CDT_PTTH,  sum(0) as CT_CDT_TDTT,  sum(0) as CT_CDT_BVMT
                        ,  sum(0) as CT_CDT_KT,  sum(0) as CT_CDT_QLNN,  sum(0) as CT_CDT_BDXH,  sum(0) as CT_CDT_KHAC
                        ,  sum(0) as CT_DT_CDT_QP,  sum(0) as CT_DT_CDT_AN,  sum(0) as CT_DT_CDT_GDDT,  sum(0) as CT_DT_CDT_KHCN,  sum(0) as CT_DT_CDT_YT
                        ...
                        ,  sum(0) as CTN
                        ,  sum(0) as CT_CCN
                        from BM86_TT343 where NGAY_HIEU_LUC >= TO_DATE ('''||to_char(TUNGAY_HL,'ddMMyyyy')||''', ''ddMMyyyy'')
                        AND NGAY_HIEU_LUC <= TO_DATE ('''||to_char(DENNGAY_HL,'ddMMyyyy')||''', ''ddMMyyyy'')     
                        and NGAY_KET_SO <=TO_DATE ('''||to_char(DENNGAY_KS,'ddMMyyyy')||''', ''ddMMyyyy'')
                        and NGAY_KET_SO >=TO_DATE ('''||to_char(TUNGAY_KS,'ddMMyyyy')||''', ''ddMMyyyy'')
                        AND NOT MA_DVQHNS LIKE ''7%''
                        ...
                        union all                         
                        select Cast(''IV'' as nvarchar2(50)) as MA_CHUONG,Cast(''CHI BỔ SUNG CÓ MỤC TIÊU CHO NGÂN SÁCH HUYỆN'' as nvarchar2(50)) as TEN_CHUONG
                        ,  sum(0) as CT_CDT_QP,  sum(0) as CT_CDT_AN,  sum(0) as CT_CDT_GDDT,  sum(0) as CT_CDT_KHCN,  sum(0) as CT_CDT_YT
                        ,  sum(0) as CT_CDT_VHTT,  sum(0) as CT_CDT_PTTH,  sum(0) as CT_CDT_TDTT,  sum(0) as CT_CDT_BVMT
                        ,  sum(0) as CT_CDT_KT,  sum(0) as CT_CDT_QLNN,  sum(0) as CT_CDT_BDXH,  sum(0) as CT_CDT_KHAC
                        ,  sum(0) as CT_DT_CDT_QP,  sum(0) as CT_DT_CDT_AN,  sum(0) as CT_DT_CDT_GDDT,  sum(0) as CT_DT_CDT_KHCN,  sum(0) as CT_DT_CDT_YT
                        ...
                        union all
                        select Cast(''V'' as nvarchar2(50)) as MA_CHUONG,Cast(''CHI CHUYỂN NGUỒN SANG NGÂN SÁCH NĂM SAU'' as nvarchar2(50)) as TEN_CHUONG
                        ,  sum(0) as CT_CDT_QP,  sum(0) as CT_CDT_AN,  sum(0) as CT_CDT_GDDT,  sum(0) as CT_CDT_KHCN,  sum(0) as CT_CDT_YT
                        ,  sum(0) as CT_CDT_VHTT,  sum(0) as CT_CDT_PTTH,  sum(0) as CT_CDT_TDTT,  sum(0) as CT_CDT_BVMT
                        ,  sum(0) as CT_CDT_KT,  sum(0) as CT_CDT_QLNN,  sum(0) as CT_CDT_BDXH,  sum(0) as CT_CDT_KHAC
                        ,  sum(0) as CT_DT_CDT_QP,  sum(0) as CT_DT_CDT_AN,  sum(0) as CT_DT_CDT_GDDT,  sum(0) as CT_DT_CDT_KHCN,  sum(0) as CT_DT_CDT_YT
