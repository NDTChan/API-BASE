create or replace PROCEDURE BM84_TT343 (P_CAP IN NVARCHAR2,P_USER_NAME IN NVARCHAR2,P_MA_DBHC IN NVARCHAR2,P_LOAI_BAOCAO IN VARCHAR2, P_LOAI_DL IN VARCHAR2,MABAOCAO IN VARCHAR2,TABLENAME_TH IN VARCHAR2,P_TABLENAME IN VARCHAR2, TUNGAY_KS IN DATE, DENNGAY_KS IN DATE, TUNGAY_HL IN DATE, DENNGAY_HL IN DATE,P_CONGTHUC IN NVARCHAR2, DONVI_TIEN NUMBER, cur OUT SYS_REFCURSOR) IS
...
--DBMS_OUTPUT.put_line ('t:=' ||  P_INSERT_C_R );
    ...
    --DBMS_OUTPUT.ENABLE (buffer_size => NULL);
     --DBMS_OUTPUT.put_line ('t:=' ||  QUERY_STR );
      QUERY_STR:='SELECT * FROM ' ||P_TABLENAME||' where 1=1 AND USERNAME = '''||P_USER_NAME||''' order by MA_CHITIEU ASC';
