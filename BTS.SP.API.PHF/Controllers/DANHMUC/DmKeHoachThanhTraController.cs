...
{
    [RoutePrefix("api/dm/phf_dmKeHoachThanhTra")]
    [Route("{id?}")]
    [Authorize]
    public class DmKeHoachThanhTraController : ApiController
    {
        public readonly IDM_SYS_TUDIENService _service;
        public readonly IUnitOfWorkAsync _unitOfWorkAsync;
        public DmKeHoachThanhTraController(IDM_SYS_TUDIENService service, IUnitOfWorkAsync unitOfWorkAsync)
        {
            _service = service;
        ...
        [Route("Paging")]
        [HttpPost]
        [CustomAuthorize(Method = "XEM", State = "phf_dmKeHoachThanhTra")]
        public async Task<IHttpActionResult> Paging(JObject jsonData)
        {
            ...
            var query = new QueryBuilder
            {
                ...
                Filter = new QueryFilterLinQ
                {
                    ...
                    Value = SP.PHF.ENTITY.CommonEnum.LOAI_SYS_TUDIEN.KEHOACHTHANHTRA.ToString()
                }
            };
            try
            {
        ...
        public IList<ChoiceObj> GetSelectData()
        {
            try
            {
                return _service.Queryable().Where(x => x.TRANG_THAI == 1 && x.LOAI_TUDIEN.Equals("KEHOACHTHANHTRA")).Select(x => new ChoiceObj()
                {
                    Value = x.MA_TUDIEN,
                    Text = x.TEN_TUDIEN,
                }).ToList();
        ...
        public HttpResponseMessage Export()
        {
            ...
            try
            {
                var listKeHoachThanhTra = new List<PHF_SYS_TUDIEN>();
                listKeHoachThanhTra = _service.Queryable().Where(x => x.LOAI_TUDIEN == SP.PHF.ENTITY.CommonEnum.LOAI_SYS_TUDIEN.KEHOACHTHANHTRA.ToString()).OrderBy(x => x.MA_TUDIEN).ToList();
                DateTime now = DateTime.Now;
                string date = now.ToString("dd-MM-yyyy");
                var urlFile = HttpContext.Current.Server.MapPath("~/UploadFile/ExcelTemp/");
                var filename = urlFile + "KeHoachThanhTra_Export_(" + date + ")_TM" + now.Ticks + ".xls";
                if (listKeHoachThanhTra.Count > 0)
                {
                    FileStream fileStream = new FileStream(filename, FileMode.OpenOrCreate);
                    using (var excelPackage = new ExcelPackage())
                    {
                        excelPackage.Workbook.Properties.Author = "SystemAccount";
                        excelPackage.Workbook.Properties.Title = "KeHoachThanhTra_Export";
                        excelPackage.Workbook.Worksheets.Add(date);
                        var workSheet = excelPackage.Workbook.Worksheets[1];
                        string TenSysTuDien = "Kế hoạch thanh tra";
                        _service.BindingDataToExcel(TenSysTuDien, workSheet, listKeHoachThanhTra);
        ...
        [HttpPost]
        [CustomAuthorize(Method = "THEM", State = "phf_dmKeHoachThanhTra")]
        ...
        [HttpPut]
        [CustomAuthorize(Method = "SUA", State = "phf_dmKeHoachThanhTra")]
        ...
        [HttpDelete]
        [CustomAuthorize(Method = "XOA", State = "phf_dmKeHoachThanhTra")]
