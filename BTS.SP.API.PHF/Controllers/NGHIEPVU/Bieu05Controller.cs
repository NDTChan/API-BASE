...
using BTS.SP.PHF.SERVICE.NV.BIEU05;
...
{
    [RoutePrefix("api/nv/bieu05")]
    [Route("{id?}")]
    public class Bieu05Controller : ApiController
    {
        private readonly IBIEU05Service _service;

        public Bieu05Controller(IBIEU05Service service)
        {
            _service = service;
        }
        [Route("GetData")]
        [HttpPost]
        public IHttpActionResult GetData(BIEU05Vm.InputPara parameter)
        {
            var result = new TransferObj();
            var data = new List<PHF_BIEU05>();
            try
            {
                data = ProcessingData(parameter);
                if (data.Count > 0)
                {
                    var p = new BIEU05Vm.Paged();
                    var obj = new BIEU05Vm.OutputData<PHF_BIEU05>();
                    var datapaged = data.OrderBy(x => x.Id).ThenBy(x=>x.MA_CHUONG).Skip((parameter.paged.currentPage - 1) * parameter.paged.itemsPerPage).Take(parameter.paged.itemsPerPage).ToList();
                    obj.lstdata = datapaged.OrderBy(x => x.MA_CHUONG).ThenBy(x => x.MA_LOAI).ThenBy(x => x.MA_KHOAN).ThenBy(x => x.MA_MUC).ThenBy(x => x.MA_TIEUMUC).ThenBy(x => x.MA_DVQHNS).ToList();
                    p = parameter.paged;
        ...
        private List<PHF_BIEU05> ProcessingData(BIEU05Vm.InputPara p)
        {
            var result = new List<PHF_BIEU05>();
            try
            {
                using (STCContext context = new STCContext())
                {
                    using (var dbContextTransaction = context.Database.BeginTransaction())
                    {
                        ...
                        var str = "BEGIN PHF_TT_BIEU05(:P_CONGTHUC, :TUNGAY_KS, :DENNGAY_KS, :TUNGAY_HL, :DENNGAY_HL,:outRef); END;";
                        ...
                        while (reader.Read())
                        {
                            ...
                            PHF_BIEU05 record = new PHF_BIEU05();
